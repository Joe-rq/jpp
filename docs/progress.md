...
**The live public site is still serving the untrimmed data today.** GitHub Pages for this repository serves from `main:/docs` (confirmed via `gh api repos/Towow-ai/jpp/pages`). Until PR #33 merges, `https://towow-ai.github.io/jpp/demos/towow/real/` continues to serve the pre-trim `data.json` with full abstracts. Merging with the one known test failure (option (c) in the PR) is therefore the only option that removes the abstracts from the live site today; options (a)/(b) additionally require the paid rerun above first. This is within `main`'s standing $5-without-asking threshold (地基 §5.2), not something that needs to wait on Nature.

[Implementation commit / 实现提交](https://github.com/Towow-ai/jpp/commit/b6738fa), [progress-log commits](https://github.com/Towow-ai/jpp/commit/1c1a22e) / [4592f18](https://github.com/Towow-ai/jpp/commit/4592f18), [PR #33](https://github.com/Towow-ai/jpp/pull/33)
