...
fn 跑(args: &[&str]) -> (String, String) {
    let o = Command::new(env!("CARGO_BIN_EXE_jpp")).args(args).output().unwrap();
    (String::from_utf8_lossy(&o.stdout).into(), String::from_utf8_lossy(&o.stderr).into())
}
...
fn 同一个程序两份档案两种严重度() {
    ...
    for (名, 值) in [("成立", false), ("不成立", true)] {
        ...
        fs::write(d.join(format!("{名}.json")), serde_json::to_string(&j).unwrap()).unwrap();
    }
    let p = d.join("p.jpp");
    let (_, e1) = 跑(&["run", p.to_str().unwrap(), "--profile", d.join("成立.json").to_str().unwrap()]);
    let (_, e2) = 跑(&["run", p.to_str().unwrap(), "--profile", d.join("不成立.json").to_str().unwrap()]);

    assert!(e1.contains("J-01") && !e1.contains("降 warn"), "H5 成立 → 挡住：{e1}");
    assert!(e2.contains("J-01") && e2.contains("降 warn"), "**同一个程序，档案说模型会算术就降 warn**：{e2}");
    assert!(e2.contains("arithmetic_capable"), "要说出是哪个字段让它降的：{e2}");
...
fn calib目录让unsure_bound不再恒等于n() {
    ...
    fs::write(cd.join("k.json"), r#"{"key":"k","hi":0.75,"lo":0.25,"n":60,"status":"上岗","unsure_rate":0.1,"set_id":"s1"}"#).unwrap();
    ...
    let (无, _) = 跑(&["run", u.to_str().unwrap(), "--fixtures", f.to_str().unwrap()]);
    let (有, _) = 跑(&["run", u.to_str().unwrap(), "--fixtures", f.to_str().unwrap(), "--calib", cd.to_str().unwrap()]);
    let 读 = |s: &str| -> (f64, i64) {
        let v: serde_json::Value = serde_json::from_str(s).unwrap();
        (v["value"]["union_bound"].as_f64().unwrap(), v["value"]["n_unknown"].as_i64().unwrap())
    };
    assert_eq!(读(&无), (2.0, 2), "没有 --calib 时它恒等于 n——第二个作者报的就是这个");
...
fn 夹具覆盖目录时要报出来() {
    ...
    fs::write(cd.join("k.json"), r#"{"key":"k","hi":0.90,"lo":0.10,"n":60,"status":"上岗"}"#).unwrap();
    fs::write(d.join("p.jpp"), "budget {calls: 2, cost: 0};\nhandle(cut(judge(state(mat(\"x\")), test(\"行吗\",\"k\"))), {act: fn(){\"act\"}, ignore: fn(){\"ig\"}, unsure: fn(u){consume(u,\"drop\");\"un\"}})\n").unwrap();
    fs::write(d.join("f.json"), r#"{"calibrations":[{"key":"k","hi":0.50,"lo":0.10,"n":60,"status":"上岗"}],"observations":[{"on":["x"],"op":"test","text":"行吗","calib":"k","answer":{"Noul":0.7}}]}"#).unwrap();
    let (out, err) = 跑(&["run", d.join("p.jpp").to_str().unwrap(), "--fixtures", d.join("f.json").to_str().unwrap(), "--calib", cd.to_str().unwrap()]);
    assert!(out.contains("\"value\": \"act\""), "0.7 过了夹具的 0.50 → 夹具赢：{out}");
    assert!(err.contains("覆盖") && err.contains("k"), "**谁盖了谁要说出来**：{err}");
    let _ = fs::remove_dir_all(&d);
}
...
fn 校准环闭合() {
    ...
    // 第一趟：没有线 → 冷；同时把证据写出去
    let (一, _) = 跑(&["run", p.to_str().unwrap(), "--fixtures", f.to_str().unwrap(),
                       "--calib-out", cd.to_str().unwrap()]);
    let v1: serde_json::Value = serde_json::from_str(&一).unwrap();
    assert_eq!(v1["value"]["r"], "cold", "没有线就是冷：{一}");
    assert!(cd.join("k.json").exists(), "**证据要落盘**");
    let rec: serde_json::Value = serde_json::from_str(&fs::read_to_string(cd.join("k.json")).unwrap()).unwrap();
    assert_eq!(rec["status"], "待真值", "无标注观察把冷记录推到待真值，**n 不动**");
    assert_eq!(rec["n"], 0);
    assert_eq!(rec["samples"].as_array().unwrap().len(), 1, "这一趟判了一次，折进一条观察");

    // 宿主侧认证（`commission` 留在宿主侧——认证是校准过程，不是程序行为）：
    // 这里直接把一条上岗线写进那个目录，模拟「校准过程给了线」
    fs::write(cd.join("k.json"), r#"{"key":"k","hi":0.8,"lo":0.2,"n":50,"status":"上岗","samples":[]}"#).unwrap();

    // 第二趟：把线读回来 → 出口变了，而且说得出线是哪来的
    let (二, _) = 跑(&["run", p.to_str().unwrap(), "--fixtures", f.to_str().unwrap(),
                       "--calib", cd.to_str().unwrap()]);
    let v2: serde_json::Value = serde_json::from_str(&二).unwrap();
    assert_eq!(v2["value"]["r"], "act", "**线上岗了，0.9 过线 → act**：{二}");
    assert_eq!(v2["value"]["源"], "题级·手填", "**`line_source` 报得出线是哪来的**");
    let _ = fs::remove_dir_all(&d);
}
...
fn calib_out的产物能原样喂回calib() {
    ...
    let (_, e1) = 跑(&["run", p.to_str().unwrap(), "--fixtures", f.to_str().unwrap(),
                       "--calib-out", cd.to_str().unwrap()]);
    assert!(cd.join("k.json").exists(), "第一趟要落盘：{e1}");
    let 原样 = fs::read_to_string(cd.join("k.json")).unwrap();

    // **中间一个字节都不改**，直接喂回去
    let (out2, err2) = 跑(&["run", p.to_str().unwrap(), "--fixtures", f.to_str().unwrap(),
                            "--calib", cd.to_str().unwrap()]);
    assert!(!err2.contains("不认得的字段"), "**写出来的必须读得回去**：{err2}");
    assert!(out2.contains("\"value\""), "第二趟要跑完：{out2} / {err2}");
    assert_eq!(fs::read_to_string(cd.join("k.json")).unwrap(), 原样, "读那一趟不该改动它");

    // 再跑一趟 out → 仍读得回去（**累积之后也要往返**）
    let (_, _) = 跑(&["run", p.to_str().unwrap(), "--fixtures", f.to_str().unwrap(),
                      "--calib", cd.to_str().unwrap(), "--calib-out", cd.to_str().unwrap()]);
    let (_, err3) = 跑(&["run", p.to_str().unwrap(), "--fixtures", f.to_str().unwrap(),
                         "--calib", cd.to_str().unwrap()]);
    assert!(!err3.contains("不认得的字段"), "累积一轮之后仍要读得回去：{err3}");
    let _ = fs::remove_dir_all(&d);
}
