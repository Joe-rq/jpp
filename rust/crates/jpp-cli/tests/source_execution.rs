...
fn forms_fill_questions_whose_fields_are_readable() {
    ...
    let trip: Vec<_> = v["trip"].as_array().unwrap().iter().map(|x| x["answer"]["answer"].as_str().unwrap().to_string()).collect();
...
fn sieve_splits_every_item_into_exactly_one_stream() {
    let report = run(&["run", "examples/sieve.jpp", "--fixtures", "examples/fixtures/sieve.json"]);
    assert_eq!(report["cost"]["calls"], 9);
    for s in report["value"]["streams"].as_array().unwrap() {
        let idx = |k: &str| -> Vec<i64> {
            s[k].as_array().unwrap().iter().map(|x| if x.is_object() { x["index"].as_i64().unwrap() } else { x.as_i64().unwrap() }).collect()
        };
        let mut all: Vec<i64> = ["act", "ignore", "unsure", "unobserved"].iter().flat_map(|k| idx(k)).collect();
        all.sort();
...
fn sieve_budget_stop_reports_unobserved_range() {
    let report = run(&["run", "examples/sieve-budget.jpp", "--fixtures", "examples/fixtures/sieve.json"]);
...
fn sieve_output_is_sieve_input_and_replays_free() {
    ...
    let report = run(&["run", "examples/sieve-compose.jpp", "--fixtures", "examples/fixtures/sieve.json", "--ledger-out", ledger.to_str().unwrap()]);
    let second = &report["value"]["second"];
    assert_eq!(second["act"].as_array().unwrap().len(), 2);
    assert_eq!(second["act"][0]["trail"], json!(["act"]));
    assert_eq!(second["ignore"], json!(["我们计划明年夏天去成都和北京旅游。"]));
    let replay = run(&["run", "examples/sieve-compose.jpp", "--fixtures", "examples/fixtures/sieve.json", "--replay", ledger.to_str().unwrap()]);
    assert_eq!(replay["cost"]["calls"], 0);
...
fn tally_counts_an_exact_interval_and_first_is_blocked_by_an_earlier_unsure() {
    let report = run(&["run", "examples/tally.jpp", "--fixtures", "examples/fixtures/tally.json"]);
    let v = &report["value"];
...
fn tally_counts_unobserved_into_the_upper_bound() {
    let report = run(&["run", "examples/tally-budget.jpp", "--fixtures", "examples/fixtures/tally.json"]);
    let v = &report["value"];
...
fn pairs_keep_both_ends_and_a_combination_pairs_again() {
    ...
    std::fs::create_dir_all(&dir).unwrap();
    let ledger = dir.join("ledger.json");
    let report = run(&["run", "examples/pair-team.jpp", "--fixtures", "examples/fixtures/pair-team.json", "--ledger-out", ledger.to_str().unwrap()]);
    let v = &report["value"];
    assert_eq!(v["round1"]["pairs"], 12);
    assert_eq!(v["round1"]["count"], json!([3, 5]));
    assert_eq!(v["round1"]["act"][0], json!({"need": "做一个带登录的网页后台", "person": "王工"}));
    assert_eq!(v["round2"]["combo_round1_exit"], json!(["act"]));
    assert_eq!(v["round2"]["act"], json!(["李工"]));
    let replay = run(&["run", "examples/pair-team.jpp", "--fixtures", "examples/fixtures/pair-team.json", "--replay", ledger.to_str().unwrap()]);
    assert_eq!(replay["cost"]["calls"], 0);
...
fn iterate_reports_why_it_stopped() {
    let report = run(&["run", "examples/iterate.jpp", "--fixtures", "examples/fixtures/iterate.json"]);
    let v = &report["value"];
...
fn fails(args: &[&str]) -> String {
    let result = Command::new(env!("CARGO_BIN_EXE_jpp")).current_dir(root()).args(args).output().unwrap();
    assert!(!result.status.success(), "应当失败：{}", String::from_utf8_lossy(&result.stdout));
...
fn every_construction_returns_the_same_contract_and_composes_again() {
    ...
    std::fs::create_dir_all(&dir).unwrap();
    let ledger = dir.join("ledger.json");
    let report = run(&["run", "examples/contract.jpp", "--fixtures", "examples/fixtures/contract.json", "--ledger-out", ledger.to_str().unwrap()]);
    let v = &report["value"];
    assert_eq!(v["same_type"], true);
    let kinds: Vec<&str> = v["stages"].as_array().unwrap().iter().map(|s| s["kind"].as_str().unwrap()).collect();
    assert_eq!(kinds, ["sieve", "pair", "sieve", "outcome", "sieve", "sieve", "tally"]);
    ...
    let replay = run(&["run", "examples/contract.jpp", "--fixtures", "examples/fixtures/contract.json", "--replay", ledger.to_str().unwrap()]);
    assert_eq!(replay["cost"]["calls"], 0);
...
fn contract_evidence_is_ledger_keys_only() {
    let report = run(&["run", "examples/contract.jpp", "--fixtures", "examples/fixtures/contract.json"]);
