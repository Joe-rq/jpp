//! B83 旁路测试（步 7c）：头行 `calib_used` 是本趟实际命中的集合，续接后只凭账本重放复现续接趟。
//!
...
fn 头行(l: &Path) -> Value {
    serde_json::from_str(fs::read_to_string(l).unwrap().lines().next().unwrap()).unwrap()
}
...
fn c_续接趟不走k_账本不含k_重放无告警() {
    ...
    assert!(k记录(&l2).is_none(), "续接趟没命中 K，L2 不留它");
    let rep = run(
...
fn d_同库续接_无告警_命中集合不变() {
    ...
    assert_eq!(头行(&l1)["calib_used"], 头行(&l2)["calib_used"]);
}
