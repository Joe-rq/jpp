...
fn b36_model_review_batch_certifies_model_labels_and_names_the_reviewer() {
    ...
    for i in 0..15 {
        rows.push(json!({"form": mention(), "item": format!("pos{i}"), "p": 0.97, "label": true, "source": "model:fable-5.1", "spot_check": "fable-r1-M"}));
        rows.push(json!({"form": mention(), "item": format!("neg{i}"), "p": 0.02, "label": false, "source": "model:fable-5.1", "spot_check": "fable-r1-M"}));
    }
    let labels = d.join("labels.jsonl");
    write_rows(&labels, &rows);
    let (report, _) = jpp(&["calib-import", labels.to_str().unwrap(), "--calib-out", d.join("calib").to_str().unwrap()]);
    assert_eq!(report[0]["status"], "上岗", "{report}");
    assert_eq!(report[0]["truth"]["gate"], "上岗（复核者 model:fable-5.1）");
    assert_eq!(report[0]["truth"]["spot_check"]["n"], 30);
    assert_eq!(report[0]["truth"]["spot_check"]["model_reviewers"], json!(["model:fable-5.1"]));
}
...
fn b36_review_from_the_same_source_as_the_annotation_is_rejected() {
    ...
    let mut rows = sonnet_rows();
    for i in 0..30 {
        rows.push(json!({"form": mention(), "item": format!("pos{i}"), "p": 0.97, "label": true, "source": "model:claude-sonnet-5", "spot_check": "self-r1"}));
    }
    let labels = d.join("labels.jsonl");
    write_rows(&labels, &rows);
    let (report, _) = jpp(&["calib-import", labels.to_str().unwrap(), "--calib-out", d.join("calib").to_str().unwrap()]);
    assert_eq!(report[0]["status"], "待真值");
    assert!(report[0]["truth"]["spot_check"].is_null(), "同源复核不计入一致率");
    assert!(report[0]["truth"]["gate"].as_str().unwrap().starts_with("待核"));
    assert!(report[0]["warnings"].to_string().contains("W-review-same-source"));
}
...
fn b36_truth_order_is_computed_then_human_then_review_then_annotation() {
    ...
    let rows = vec![
        ...
        json!({"form": mention(), "item": "a", "p": 0.5, "label": false, "source": "model:fable-5.1", "spot_check": "fable-r1-M"}),
        // b：标注、复核、人工 → 取人工
        json!({"form": mention(), "item": "b", "p": 0.5, "label": true, "source": "model:claude-sonnet-5"}),
        json!({"form": mention(), "item": "b", "p": 0.5, "label": true, "source": "model:fable-5.1", "spot_check": "fable-r1-M"}),
    ...
    write_rows(&labels, &rows);
    let (report, _) = jpp(&["calib-import", labels.to_str().unwrap(), "--calib-out", d.join("calib").to_str().unwrap()]);
    assert_eq!(report[0]["truth"]["sources"], json!({"computed": 1, "human": 1, "model:fable-5.1": 1}));
...
fn cost_line_form_cert_replays_from_ledger_only() {
    use jpp_core::effects::{CalibStore, Cert, LabelSource};
    ...
    jpp(&["run", hash_src.to_str().unwrap(), "--output", hash_out.to_str().unwrap()]);
    ...
    for (key, cost) in [("k", None), (fk.as_str(), Some((1.0, 5.0)))] {
        c.put(key, 0.8, 0.2, 50, "上岗").unwrap();
        let cert = Cert { alpha: 0.10, conf_delta: 0.10, hi: 0.7, n_accepted: 50, n_errors: 0, ucb: 0.05,
            cluster_unit: "测试合成证书".into(), resample: None, cost, bounded_side: "单侧".into(),
            label_fp: String::new(), selection: None, grade: Default::default(), label_source: LabelSource::全体 };
    ...
    let ledger = d.join("ledger.json");
    let first = d.join("first.json");
    jpp(&["run", src.to_str().unwrap(), "--fixtures", fixtures.to_str().unwrap(), "--calib", calib.to_str().unwrap(),
          "--ledger-out", ledger.to_str().unwrap(), "--output", first.to_str().unwrap()]);
    let first: Value = serde_json::from_slice(&fs::read(&first).unwrap()).unwrap();
    assert_eq!(first["value"], json!("act"), "{first}");

    let head: Value = serde_json::from_str(fs::read_to_string(&ledger).unwrap().lines().next().unwrap()).unwrap();
    assert!(head["calib_used"].get(&fk).is_some(), "题式键要入账：{}", head["calib_used"]);

    let again = d.join("again.json");
    jpp(&["run", src.to_str().unwrap(), "--replay", ledger.to_str().unwrap(), "--output", again.to_str().unwrap()]);
    let again: Value = serde_json::from_slice(&fs::read(&again).unwrap()).unwrap();
    assert_eq!(again["cost"]["calls"], 0);
    assert_eq!(again["value"], first["value"]);
}
