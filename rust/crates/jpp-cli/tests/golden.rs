...
//! 录制或按 S 步预注册更新：`JPP_GOLDEN_UPDATE=1 cargo test -p jpp-cli --test golden`。
...
fn check(dir: &Path, file: &str, actual: &str, diffs: &mut Vec<String>) {
    ...
    match fs::read_to_string(&path) {
        ...
        Err(_) => diffs.push(format!(
            "{}：缺金样。修法：JPP_GOLDEN_UPDATE=1 cargo test -p jpp-cli --test golden 录制，并在提交说明里写明原因",
            path.display()
        )),
    }
}
...
/// `ir.txt` 是新增产物，只要求与自身一致，不参与行为判定。步 12d 起 IR 由 `jpp_core::lower` 直接产出。
/// 预期报错的示例：能降级的照样打印；降级报带规则号的诊断（缺预算 `J-07a`）的打印诊断；语法错的不产出。
#[test]
fn ir_is_wellformed_and_prints_stably() {
    use jpp_core::names::CurrentNames;
    use jpp_core::ir;
    let golden = root().join("tests/golden");
    let mut diffs = vec![];
    for c in cases() {
        ...
        let (first, annotated) = match jpp_core::lower(&loaded.program) {
            Ok(prog) => {
                ...
                assert_eq!(ir::print(&prog, None), ir::print(&prog, None), "{}：IR 两次打印不同", c.name);
                let annotated = match &built {
                    Ok(()) => {
                        let (_, annot) = jpp_core::check::check_annotated(&prog, None, None);
                        let text = ir::print(&prog, Some(&annot));
                        let (_, again) = jpp_core::check::check_annotated(&prog, None, None);
                        assert_eq!(text, ir::print(&prog, Some(&again)), "{}：标注打印两次不同", c.name);
                        text
                    }
                    Err(_) => first.clone(),
                };
