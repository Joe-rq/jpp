...
//! 提示由 `run_io` 无条件打印。
use crate::options::{Backend, DEFAULT_LIVE_MODEL, RunOptions};
...
/// 这一趟是否用真机后端发调用（首跑或续接）。重放不算：它不初始化真机客户端。
pub fn uses_live_backend(options: &RunOptions) -> bool {
    options.backend == Backend::Live && options.replay.is_none()
}
...
fn load(path: &Path) -> Result<Resolved, String> {
    let profile = Profile::load(path).map_err(|e| format!("{}: {e}", path.display()))?;
    Ok(Resolved {
...
pub fn resolve(options: &RunOptions) -> Result<Option<Resolved>, String> {
    ...
    Err(format!(
        ...
        "E-profile-missing: --backend live 需要模型 {model} 的能力画像（B73），不回退代码兜底值；试过：{tried_text}。\
