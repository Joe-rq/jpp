...
/// 检查器诊断 → 待呈现的诊断
pub fn from_check(d: &jpp_core::Diagnostic) -> Item {
    let level = match d.severity {
        jpp_core::Severity::Error => Level::Error,
        jpp_core::Severity::Warning => Level::Warning,
    };
    Item::new(
        d.rule.clone(),
        level,
...
pub fn from_runtime(e: &jpp_core::RtError) -> Item {
    Item::new(
...
mod tests {
    ...
    #[test]
    fn 编号表覆盖运行期全部编号() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../jpp-core/src/interp");
