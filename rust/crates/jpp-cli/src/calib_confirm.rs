...
mod tests {
    ...
    fn exact_key_beats_alias() {
        ...
        assert_eq!(resolve_key(&s, "\u{1f}form\u{1f}abc").unwrap(), "\u{1f}form\u{1f}abc");
    }
    ...
    fn missing_key_keeps_old_message() {
        ...
        let dir = std::env::temp_dir().join(format!("jpp-calib-confirm-missing-{}", std::process::id()));
    ...
    fn suspend_touches_only_exact_record() {
        let dir = std::env::temp_dir().join(format!("jpp-calib-confirm-exact-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        store(&["form:abc", "\u{1f}form\u{1f}abc"]).save(&dir).unwrap();
