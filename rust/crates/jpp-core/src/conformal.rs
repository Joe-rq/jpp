...
pub fn drift(reference: &[f64], recent: &[f64], bins: usize) -> DriftReport {
    ...
    DriftReport {
        ks, psi, n_ref: reference.len(), n_recent: recent.len(),
        significant: ks >= crit,
...
pub fn cost_line(samples: &[(f64, bool)], fp: f64, fn_: f64) -> Result<CostLine, String> {
    ...
    for t in cands {
        ...
        if best_c.is_none() || c < best_c.expect("已判") || (c == best_c.expect("已判") && t > best_t) {
            best_t = t;
            best_c = Some(c);
        }
    }
    ...
    Ok(CostLine {
        ...
        false_accept_rate: if acc.is_empty() { None } else { Some(误放行 as f64 / acc.len() as f64) },
    })
}
