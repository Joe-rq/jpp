...
use crate::effects::CalibStore;
use crate::value::{Answer, Reading};
...
fn probability(r: &Reading) -> f64 {
    if r.fail.is_some() {
        return 0.0;
    }
    match r.answer_after_flush() {
        Some(Answer::Noul(p)) => p,
        Some(Answer::Choice(v)) | Some(Answer::Score(v)) => v.iter().fold(f64::MIN, |a, b| if *b > a { *b } else { a }),
        None => 0.0,
    }
}
...
/// **是两边同一个洞**，Rust 这边先补。
pub fn uncertainty(calib: &CalibStore, r: &Reading) -> Option<f64> {
    ...
    let delta = calib.delta_for(&rec, r.op);
    let (hi, lo) = calib.lines_for(&rec);
    let p = probability(r);
    let (top, bot) = (hi + delta, lo - delta);
    Some(if bot <= p && p <= top { 0.0 } else { -(p - top).abs().min((p - bot).abs()) })
}
...
pub fn allocate(calib: &CalibStore, readings: &[Rc<Reading>], k: usize) -> Vec<usize> {
    allocate_report(calib, readings, k).picked
}

/// 同上，外加「哪些排不了序」。**`allocate` 是它只取 `picked` 的特例。**
pub fn allocate_report(calib: &CalibStore, readings: &[Rc<Reading>], k: usize) -> Allocation {
    let mut scored: Vec<(usize, f64)> = vec![];
    let mut 算不出: Vec<usize> = vec![];
    for (i, r) in readings.iter().enumerate() {
        match uncertainty(calib, r) {
            Some(u) => scored.push((i, u)),
            None => 算不出.push(i),
        }
    }
    scored.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal).then(a.0.cmp(&b.0)));
    Allocation { picked: scored[..k.min(scored.len())].iter().map(|(i, _)| *i).collect(), 算不出 }
}
...
/// 并在界里按 1 计。
pub fn unsure_bound(calib: &CalibStore, readings: &[Rc<Reading>]) -> UnsureBound {
    let mut us = vec![];
