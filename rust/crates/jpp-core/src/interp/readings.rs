...
impl<'a> Interp<'a> {
    ...
    pub(in crate::interp) fn answer_of(&self, r: &Reading) -> Option<Answer> {
        self.answers.borrow().answer_of(r)
    }

    /// 新读数的句柄（本次运行内唯一）
    pub(in crate::interp) fn new_reading_id(&self) -> u64 {
...
pub(in crate::interp) enum RefreshKind {
...
pub(in crate::interp) const REFRESH_POINTS: &[(&str, RefreshKind, &str)] = &[
...
/// 查一个刷新原因是否已登记。
pub(in crate::interp) fn refresh_point(reason: &str) -> Option<RefreshKind> {
    REFRESH_POINTS
        .iter()
        .find(|(r, _, _)| *r == reason)
        .map(|(_, k, _)| *k)
