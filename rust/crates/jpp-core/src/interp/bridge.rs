...
impl<'a> Interp<'a> {
    ...
    pub(in crate::interp) fn order_tiers(&self, rs: &[Rc<Reading>]) -> Vec<Vec<usize>> {
        ...
        let delta = rs
            .first()
            .map(|r| self.calib.delta_for(&self.calib.line(&r.calib), r.op))
            .unwrap_or(0.0);
    ...
    pub(in crate::interp) fn new_exit(
        &mut self,
        kind: ExitKind,
    ...
    pub(in crate::interp) fn new_exit_from(
        &mut self,
        kind: ExitKind,
        untested: Option<String>,
        op: Op,
    ...
    pub(in crate::interp) fn readings_of(
        &self,
        v: &Value,
        who: &str,
        sp: Span,
    ...
    pub(in crate::interp) fn 报漂移(&mut self, key: &str, sp: Span) {
    ...
    pub(in crate::interp) fn 报漂移_批(&mut self, rs: &[Rc<Reading>], sp: Span) {
    ...
    /// 重放时它照样查不到，照样是冷，出口一致。
    pub(in crate::interp) fn note_calib(&mut self, key: &str) {
        ...
        if let Some(j) = self.calib.record_json(key) {
            ...
            self.ledger
                .calib_used
                .insert(key.to_string(), serde_json::json!({"hash": h, "record": j}));
        }
    }

    /// 题式级记录：查到就记进账本（`calib_used`），再取记录。代价分支与非代价分支共用，避免再漏记
    /// （补丁请求 3：只凭账本重放要补回同一条线）。
    pub(in crate::interp) fn 查题式(&mut self, r: &Reading) -> Option<Lookup> {
    ...
    pub(in crate::interp) fn 查类(&mut self, key: &str) -> Option<Lookup> {
        let c = self.calib.chain(key, None).class?;
        self.note_calib(&c.key);
        Some(c.rec)
    }

    pub(in crate::interp) fn cut(
        &mut self,
        r: &Reading,
            ...
            *e.ledger_key.borrow_mut() = r.ledger_key.clone();
            ...
            if self.exit_grades.len() == 已记 {
                self.exit_grades.push(serde_json::json!({
                    "site": sp.start, "exit": e.label(), "grade": "Cold", "releases": false,
                }));
            }
        }
        Ok(v)
    ...
    pub(in crate::interp) fn cut_inner(
        &mut self,
        r: &Reading,
        calib_key: Option<&str>,
        cost: Option<(f64, f64)>,
            ...
            || rec.status == "停岗候选"
        {
            ...
            if let Some(t) = f.truth_gate.as_ref().filter(|g| g.starts_with("临时上岗")) {
                self.trace
                    .warn(format!("W-provisional: @{} 这条题式级线是{}", sp.start, t));
            }
            (
                Some((f.hi, f.lo)),
                format!("题式级·{}", 凭据(&f)),
            ...
            if let Some(t) = c.truth_gate.as_ref().filter(|g| g.starts_with("临时上岗")) {
                self.trace
                    .warn(format!("W-provisional: @{} 这条类级线是{}", sp.start, t));
            }
            (
                Some((c.hi, c.lo)),
                format!("类级·{}", 凭据(&c)),
        ...
        let delta = self.calib.delta_for(&所用链(&线源).rec, r.op);
        ...
        if let Value::Exit(e) = &出口 {
            ...
            // 依据：B75（类线出口可路由、不放行不可逆 do；`Exit::class_line` 注释）
            e.class_line
                .set(线源.starts_with("类级") && !e_line_empty(&e.line_source));
            ...
            if 试用 {
                ...
                // 依据：B72（试用线出口可路由、报 W-trial-line，文本带 α 与 n；步 17b 前等级不随材料传递）
                self.trace.warn(format!(
                    "W-trial-line: @{} 键 {} 的线是试用线（α={}，n={}，认证半上侧已决 {}），出口 {} 照常路由，按 B72 不算放行不可逆 do 的可信合取项；等级不随材料传递（步 17b 前：由本出口选出的材料在下一跳再判断时，下一跳的出口不继承试用等级）。修法【作者可改】：补足带真值的标注（字面题式约 160 条）经 calib-import 认证正式线",
                    sp.start,
                    所用链(&线源).key,
                    c.alpha,
                    所用链(&线源).rec.n,
            ...
            if !e_line_empty(&e.line_source) {
                if let (Some(sc), Some(fp)) = (用链.rec.scope.as_ref(), r.fp.as_ref()) {
                    if let Some((量, v, lo, hi)) = sc.outside(fp) {
                        ...
                        self.trace.warn(format!(
                            ...
                            sp.start, sc.n, e.kind
                        ));
                    }
                }
            }
            // 逐出口记线等级（步 20f）：优先序 Cold > Fixture > Class > Trial > Provisional > Form > Certified
            // （`20` §3.4 等级表；多个成立时取靠前者）。出口不进账本，重放时按账本头的记录重算出同一张表。
            ...
            let grade = if !用线 {
                "Cold"
            } else if e.fixture_line.get() {
                "Fixture"
            } else if e.class_line.get() {
                "Class"
            } else if e.trial_line.get() {
                "Trial"
            } else if 临时 {
                "Provisional"
            ...
            // releases：运行时实际的放行判定去掉 taint 一项（`guard_trusted` 的等级位与正交位）
            let releases = 用线
                && !e.fixture_line.get()
                && !e.suspend_candidate.get()
                && !e.scope_out.get()
                && !e.class_line.get()
                && !e.trial_line.get();
            let mut rec = serde_json::json!({
                "site": sp.start, "exit": e.label(), "grade": grade, "releases": releases,
            });
            if 用线 {
