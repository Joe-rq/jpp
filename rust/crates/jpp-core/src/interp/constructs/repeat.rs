...
impl<'a> Interp<'a> {
    ...
    pub(in crate::interp) fn b_agg(
        ...
        let merged = merge_runs(&*self.answers.borrow(), &rs, &method, sp)?;
        let n_runs = rs.len();
        ...
        if self.ledger.get(&lk).is_none() {
            self.ledger.put(Entry::effect_keyed(lk.clone(), "repeat", serde_json::json!({"n": n_runs, "method": method, "calib": 键.replace('\u{1f}', ":")}), 0.0));
        }
        self.trace.push(
            "repeat",
            &lk,
        ...
        self.fill_answer(&r, merged);
        Ok(Value::Reading(r))
    }
    #[allow(unused_variables)]
    pub(in crate::interp) fn b_order(
        &mut self,
        name: &'static str,
        args: Vec<Value>,
        sp: Span,
