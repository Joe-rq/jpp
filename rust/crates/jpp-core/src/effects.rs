    ...
    fn judge(&mut self, state: &State, questions: &[&Question]) -> Result<JudgeResult, EffectError> {
        ...
        for q in questions {
            ...
            let a = self.table.get(&k).cloned().ok_or_else(|| {
                EffectError(format!("固定观察未命中：题「{}」× 状态 {}（键 {k}）；固定观察不是模型，缺记录即错", q.text, &state.hash[..8]))
            })?;
        ...
        Ok(JudgeResult { answers, tokens: 0, cost: 0.0 })
    }
    fn generate(&mut self, prompt: &str, _ctx: &[Json], _n: usize, retry_seq: u64) -> Result<Vec<Json>, EffectError> {
        self.n_calls += 1;
        self.gens.get(&format!("{prompt}\u{1f}{retry_seq}")).cloned().ok_or_else(|| EffectError(format!("固定生成未命中：{prompt} retry_seq={retry_seq}")))
    }
    fn ask(&mut self, state: &State, q: &Question) -> Result<Option<Answer>, EffectError> {
...
impl Client for NoCallClient {
    ...
    fn generate(&mut self, p: &str, _c: &[Json], _n: usize, _r: u64) -> Result<Vec<Json>, EffectError> {
...
impl JevClient {
    pub fn with_transport(model: &str, transport: Box<dyn FnMut(&Json) -> Result<Json, EffectError>>) -> JevClient {
        JevClient { model: model.to_string(), usd_per_input_token: 4.2e-8, n_calls: 0, transport }
    }
    #[cfg(feature = "live")]
...
impl Client for JevClient {
    ...
    fn judge(&mut self, state: &State, questions: &[&Question]) -> Result<JudgeResult, EffectError> {
        ...
        let tokens = resp.get("usage").and_then(|u| u.get("input_tokens")).and_then(|t| t.as_u64()).unwrap_or(0);
        self.n_calls += 1;
        Ok(JudgeResult { answers, tokens, cost: tokens as f64 * self.usd_per_input_token })
    }
    fn generate(&mut self, _p: &str, _c: &[Json], _n: usize, _r: u64) -> Result<Vec<Json>, EffectError> {
...
impl CalibStore {
    ...
    pub fn put(&mut self, key: &str, hi: f64, lo: f64, n: u64, status: &str) -> Result<(), String> {
        ...
        self.records.insert(key.to_string(), CalibRecord { key: key.into(), hi, lo, n, status: status.into(), delta: 0.05 });
        Ok(())
    }
    pub fn get(&self, key: &str) -> CalibRecord {
        self.records.get(key).cloned().unwrap_or(CalibRecord { key: key.into(), hi: 0.65, lo: 0.35, n: 0, status: "冷".into(), delta: 0.05 })
    }
}
