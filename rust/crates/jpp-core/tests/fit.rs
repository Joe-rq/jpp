...
/// 按状态顺序给不同的 p（判断向量的测试要每个对象各一个值）
struct 多态客户端 {
    ps: Vec<f64>,
    下一个: RefCell<usize>,
}
impl Client for 多态客户端 {
    fn model_id(&self) -> String {
        "fixed-0".into()
    }
    fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
        let mut i = self.下一个.borrow_mut();
        let answers = qs
            .iter()
            .map(|_| {
                let p = *self.ps.get(*i).unwrap_or(&0.5);
                *i += 1;
                Answer::Noul(p)
            })
            .collect();
        Ok(JudgeResult { answers, tokens: 0, cost: 0.0, perms: vec![], mode_share: vec![]  })
    }
    fn generate(&mut self, _p: &str, _c: &[Json], _n: usize, _r: u64) -> Result<GenResult, EffectError> {
        Err(EffectError("不该 gen".into()))
    }
...
/// 给 choice 答案 + 可选的置换众数占比（`None` = 这条路上没测过置换）
struct Choice客户端 {
    probs: Vec<f64>,
    mode_share: Option<f64>,
}
impl Client for Choice客户端 {
    fn model_id(&self) -> String {
        "fixed-0".into()
    }
    fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
        Ok(JudgeResult {
            answers: qs.iter().map(|_| Answer::Choice(self.probs.clone())).collect(),
            tokens: 0,
            cost: 0.0,
            mode_share: qs.iter().map(|_| self.mode_share).collect(),
...
fn 跑_choice(src: &str, probs: Vec<f64>, mode_share: Option<f64>) -> jpp_core::Outcome {
    let program = lower(&parse(src).expect("解析")).expect("lower");
    let mut calib = CalibStore::new();
    calib.put("k", 0.65, 0.35, 100, "上岗").unwrap();
    let mut client = Choice客户端 { probs, mode_share };
    let mut ledger = Ledger::new();
    run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger)
        .unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()))
}

fn 跑_多态(src: &str, ps: &[f64]) -> jpp_core::Outcome {
    let program = lower(&parse(src).expect("解析")).expect("lower");
    let mut calib = CalibStore::new();
    calib.put("k", 0.65, 0.35, 100, "上岗").unwrap();
...
fn 跑(src: &str, p: f64) -> jpp_core::Outcome {
    let program = lower(&parse(src).expect("解析")).expect("lower");
    let mut calib = CalibStore::new();
    calib.put("k", 0.65, 0.35, 100, "上岗").unwrap();
...
{出口: exit_kind(e)}
"#;
    assert_eq!(跑(src, 0.95).value_json()["出口"], serde_json::json!("act"), "证据齐了就该照常过线");
}
...
"#;
    ...
    let mut client = 定值客户端 { p: 0.95, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = run(&program, &mut client, &calib, &actions, &mut ledger).expect("跑完");
    assert_eq!(out.value_json()["出口"], serde_json::json!("unsure(insufficient:ctx)"), "insufficient 先判：{:?}", out.value_json());
}
...
"#;
    ...
    assert_eq!(档.as_array().map(|a| a.len()), Some(3), "两个正常读数各一档 + 失败的一档：{档:?}");
...
"#;
    ...
    let mut ledger = Ledger::new();
    let e = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect_err("读数不能求和");
...
{结果: r}
"#;
    let report = jpp_core::check(&lower(&parse(src).expect("解析")).expect("lower"));
    let d = report
        .find("J-12")
        .unwrap_or_else(|| panic!("Fail 带到程序边界而没处理该报 J-12：\n{}", report.render()));
    assert!(d.message.contains("is_fail") || d.message.contains("处理"), "报文要给修法：{}", d.message);
}
...
"#,
        ...
        let report = jpp_core::check(&lower(&parse(src).expect("解析")).expect("lower"));
        assert!(report.find("J-12").is_none(), "处理过或没带出去的不该报：\n{src}\n{}", report.render());
    }
}
...
#[test]
fn fit的结果仍是读数还要过线() {
    use jpp_core::effects::FitRegistry;
...
"#;
    let program = lower(&parse(src).expect("解析")).expect("lower");
    let mut calib = CalibStore::new();
    calib.put("t.全过", 0.6, 0.3, 60, "上岗").unwrap();
    calib.put("d.局部", 0.6, 0.3, 60, "上岗").unwrap();
    calib.put("fit.绿且局部", 0.6, 0.3, 80, "上岗").unwrap();
    calib.set_set_id("fit.绿且局部", "conf-B").unwrap();
    let mut fits = FitRegistry::new();
    // 两个特征：(校准键, 指纹种类)；训练集 train-A ≠ 保形集 conf-B
    fits.register("绿且局部", &[("t.全过", "noul"), ("d.局部", "noul")], 60, "train-A", |ps| ps[0] * ps[1]);
    let mut client = 多态客户端 { ps: vec![0.9, 0.9], 下一个: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = jpp_core::run_with_fits(&program, &mut client, &calib, &ActionRegistry::new(), &fits, &mut ledger)
        .unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()));

    // 0.9 × 0.9 = 0.81 > hi 0.6 → act。关键是它**过了线**，而不是直接给个分数
    assert_eq!(out.value_json()["出口"], serde_json::json!("act"), "fit 的结果要经 cut 判出出口：{:?}", out.value_json());
}
...
"#;
    ...
    calib.put("t.全过", 0.6, 0.3, 60, "上岗").unwrap();
    calib.put("fit.绿且局部", 0.6, 0.3, 80, "上岗").unwrap();
    let mut client = 多态客户端 { ps: vec![0.9], 下一个: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let e = jpp_core::run_with_fits(&program, &mut client, &calib, &ActionRegistry::new(), &FitRegistry::new(), &mut ledger)
...
"#;
    ...
    for k in ["别的键", "d.局部", "fit.绿且局部"] {
        calib.put(k, 0.6, 0.3, 60, "上岗").unwrap();
    }
    let mut fits = FitRegistry::new();
    fits.register("绿且局部", &[("t.全过", "noul"), ("d.局部", "noul")], 60, "train-A", |ps| ps[0] * ps[1]);
    let mut client = 多态客户端 { ps: vec![0.9, 0.9], 下一个: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let e = jpp_core::run_with_fits(&program, &mut client, &calib, &ActionRegistry::new(), &fits, &mut ledger)
        .expect_err("指纹不符该被拒");
...
fn 注册样本数不够不认() {
    ...
    let e = fits.register_checked("绿且局部", &[("a", "noul"), ("b", "noul")], 40, "train-A", |ps| ps[0] * ps[1]);
    assert!(e.is_err(), "40 < max(50, 20×2) 该被拒");
    assert!(fits.register_checked("绿且局部", &[("a", "noul"), ("b", "noul")], 50, "train-A", |ps| ps[0] * ps[1]).is_ok());
    // 三个特征 → 至少 60
    assert!(fits.register_checked("三特征", &[("a", "noul"), ("b", "noul"), ("c", "noul")], 50, "train-A", |ps| ps[0]).is_err());
}
...
"#;
    ...
    let mut client = 多态客户端 { ps: vec![0.9, 0.5], 下一个: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let e = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect_err("跨题排序该被拒");
    assert!(e.render().contains("J-04"), "该是 J-04：{}", e.render());
}
...
"#;
    ...
    let mut client = 多态客户端 { ps: vec![0.9, 0.5], 下一个: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect("同题该跑通");
...
"#;
    ...
    assert_eq!(out.value_json()["出口"], serde_json::json!("pick(0)"), "置换一致该给 Pick：{:?}", out.value_json());
...
#[test]
fn fit结果不借模式级先验() {
    use jpp_core::effects::{CalibStore, FitRegistry, LiteralMode};
    let src = r#"
budget {calls: 4, cost: 1, depth: 8};
let 甲 = judge(state(mat("代码")), test("测试全过吗", "t.全过"));
let 乙 = judge(state(mat("代码")), test("改动局部吗", "d.局部"));
...
"#;
    ...
    calib.put(&CalibStore::mode_key("noul", LiteralMode::default()), 0.80, 0.20, 200, "上岗").unwrap();
    let mut fits = FitRegistry::new();
    fits.register("绿且局部", &[("t.全过", "noul"), ("d.局部", "noul")], 60, "train-A", |ps| ps[0] * ps[1]);
    let mut client = 多态客户端 { ps: vec![0.99, 0.99], 下一个: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = jpp_core::run_with_fits(&program, &mut client, &calib, &ActionRegistry::new(), &fits, &mut ledger).unwrap();
    let v = out.value_json();
    assert_eq!(v["出口"], serde_json::json!("unsure(cold|untested:calib_line)"), "**fit 的结果不该借裸 noul 那一类的线**：{v}");
