...
impl Client for RuleClient {
    fn model_id(&self) -> String {
        "fixed-0".into()
    }
    fn judge(&mut self, state: &State, questions: &[&Question]) -> Result<JudgeResult, EffectError> {
        let on = state.on.first().and_then(|m| m.content.as_str()).unwrap_or("").to_string();
        let p = *self.table.get(&on).ok_or_else(|| EffectError(format!("固定规则里没有这一段：{on}")))?;
        self.calls += 1;
        Ok(JudgeResult { answers: questions.iter().map(|_| Answer::Noul(p)).collect(), tokens: 0, cost: 0.0, perms: vec![], mode_share: vec![]  })
    }
    fn generate(&mut self, p: &str, _c: &[Json], _n: usize, _r: u64) -> Result<jpp_core::effects::GenResult, EffectError> {
...
fn 跑一次() -> (Vec<usize>, Vec<String>, Vec<(String, f64)>) {
    ...
    calib.set_unsure_rate(CALIB, 0.2).expect("标注集上实测的 unsure 率");
    let mut client = RuleClient { table: 段落.iter().map(|(t, p, _)| (t.to_string(), *p)).collect(), calls: 0 };
    let mut ledger = Ledger::new();
    let outcome = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger)
        .unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()));

    let Some(Value::Record(fields)) = &outcome.value else { panic!("返回的该是记录，拿到 {:?}", outcome.value) };
    let get = |k: &str| fields.iter().find(|(n, _)| n == k).map(|(_, v)| v.clone()).unwrap_or_else(|| panic!("返回里缺 {k}"));
...
fn 按不确定性分配复核名额比随机分配错得少() {
    ...
    sorted.sort();
    assert_eq!(sorted, vec![2, 4, 6, 8], "最不确定的四段正是四段 fuzzy，实际选了 {picked:?}；出口 {kinds:?}");
    ...
    println!("错误率·按不确定性={e_alloc} 随机(种子0)={e_rand0} 随机(200 种子平均)={e_rand_avg} 不复核={e_none}");
    println!("J-10 上界：{bound:?}");

    // 预测 1、3、4
    assert_eq!(e_alloc, 0.0, "复核了全部四段 unsure，不该再有错");
    assert_eq!(e_none, 0.333, "不复核时四段 unsure 全记错 = 4/12");
    assert!(e_alloc < e_rand_avg, "按不确定性分配该比随机好：{e_alloc} vs {e_rand_avg}");
    assert!(e_rand_avg <= e_none, "随机复核不该比不复核更差：{e_rand_avg} vs {e_none}");
    // 预测 2：随机臂的期望 ≈ 2.67/12 ≈ 0.222
    assert!((e_rand_avg - 0.222).abs() < 0.03, "随机臂 200 种子平均应当落在 0.222 附近，实际 {e_rand_avg}");
...
fn 长处构件不花钱() {
    ...
    let mut calib = CalibStore::new();
    calib.put(CALIB, 0.65, 0.35, 100, "上岗").unwrap();
    calib.set_unsure_rate(CALIB, 0.2).unwrap();
    let mut client = RuleClient { table: 段落.iter().map(|(t, p, _)| (t.to_string(), *p)).collect(), calls: 0 };
    let mut ledger = Ledger::new();
    let outcome = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect("跑完");
    assert_eq!(outcome.cost.calls, 12, "十二段十二次判断；allocate 与 unsure_bound 一次都不该加");
    assert_eq!(outcome.cost.usd, 0.0, "固定观察零花费");
    assert!(outcome.pending.is_empty(), "不该挂起：{:?}", outcome.pending);
}
...
"#;
    ...
    let mut client = RuleClient { table: [("a".to_string(), 0.9)].into_iter().collect(), calls: 0 };
    let mut ledger = Ledger::new();
    let e = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect_err("出口不是读数，该报错");
    let text = e.render();
...
///
/// 代码路径一直是成立的（`Client::judge(state, &[&Question])`、一次 `charge`、一次 `calls += 1`），
...
"#;
    ...
    let mut calib = CalibStore::new();
    calib.put(CALIB, 0.65, 0.35, 100, "上岗").unwrap();
    let mut client = RuleClient { table: [("一段文字".to_string(), 0.9)].into_iter().collect(), calls: 0 };
    let mut ledger = Ledger::new();
    let outcome = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger)
        .unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()));

    assert_eq!(outcome.value_json(), serde_json::json!({"题数": 3}), "三道题三条读数");
    assert_eq!(outcome.cost.calls, 1, "三道题问的是同一个状态，应当**一次调用**问完（12 §10 G2 / P5）");
    assert_eq!(client.calls(), 1, "客户端那边也只该被叫一次");
    assert_eq!(outcome.trace.count("judge", false), 3, "账本按题记三条，调用只有一次——记账粒度是题，调用粒度是状态");
}
