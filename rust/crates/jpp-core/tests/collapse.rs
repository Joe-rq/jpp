...
fn 把lo推到零不算收紧() {
    ...
    c.put("k", 0.99, 0.0, 30, "停岗").expect("停岗不受收紧门管");
    c.put("k", 0.90, 0.10, 30, "上岗").expect_err("从停岗改上岗要凭据");

    // 造一条两侧都可达的上岗线（没有带标注证据的键不受门管）
    let mut d = CalibStore::new();
    d.put("k", 0.80, 0.20, 5, "上岗").unwrap();
    装(&mut d, "k"); // 现在它有带标注的证据了

    // 抬 hi（少放行）+ 保持 lo → 是收紧，免凭据
    d.put("k", 0.90, 0.20, 35, "上岗").expect("两侧都不扩大放行，免凭据");
    // **把 lo 推到 0 → `Ignore` 不可达 → 不是收紧**
    let e = d.put("k", 0.90, 0.0, 35, "上岗").expect_err("**使 Ignore 不可达不是收紧**");
    assert!(e.contains("不可达") || e.contains("Ignore"), "要说清是哪一种出口没了：{e}");
}
...
fn 把hi推到一也不算收紧() {
    ...
    d.put("k", 0.80, 0.20, 5, "上岗").unwrap();
    装(&mut d, "k");
    let e = d.put("k", 1.0, 0.20, 35, "上岗").expect_err("**使 Act 不可达也不是收紧**");
...
fn 认证之后不可达的那一侧要说得出来() {
    ...
    // **算出来的，不是填的**
    assert_eq!(rec.不可达出口(), vec!["ignore"], "认证之后 Ignore 不可达，记录要说得出来");
    assert!(cert.bounded_side.contains("lo"), "证书上那句也还在：{}", cert.bounded_side);

    // 两侧都可达的线上，这一栏是空的
    let mut d = CalibStore::new();
    d.put("k", 0.8, 0.2, 5, "上岗").unwrap();
...
"#);
        ...
        run(&program, &mut 桩, &calib, &a, &mut l).expect("跑得完").value_json()
    };
    assert_eq!(跑("取内部")["来源"], "trusted");
    assert_eq!(跑("取外部")["来源"], "untrusted",
...
"#);
    assert_eq!(ok.as_deref(), Ok("\"发了\""), "**合取一个真的可信判断就该放行**：{ok:?}");
}
...
"#;
    ...
    let 进了账本 = l.entries.iter().any(|e| matches!(e, Entry::Effect { kind, .. } if kind == "transform"));
    if out.trace.warnings.iter().any(|w| w.starts_with("W-no-cache")) {
        assert!(!进了账本, "W-no-cache 那条路确实不进账本——缺口钉住");
        let w = out.trace.warnings.iter().find(|w| w.starts_with("W-no-cache")).unwrap();
        assert!(w.contains("不进账本"), "**告警要说全**：不只是「不进跨运行缓存」：{w}");
    } else {
        // **不许留成一条「没走到也算过」的测试**——那正是今晚拆了一整夜的形状。
        panic!("没有触发 W-no-cache，这个程序证不了任何事：{:?}", out.trace.warnings);
    }
}
