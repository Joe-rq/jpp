...
"#).expect("解析")).expect("lower");
    ...
    c.put("k", hi, lo, 73, "上岗").unwrap();
    c.profile = Profile { delta: (delta, delta, delta), ..Profile::default() };
    let mut l = Ledger::new();
    let o = jpp_core::run(&program, &mut 定值(p), &c, &ActionRegistry::new(), &mut l).expect("跑得完");
...
fn 两内核对同一读数给同一出口() {
    ...
    let (hi, lo, delta) = (gold["hi"].as_f64().unwrap(), gold["lo"].as_f64().unwrap(), gold["delta"].as_f64().unwrap());
    let mut 分岔 = vec![];
    ...
    分岔.sort();
    assert!(分岔.is_empty(), "**{} 格分岔**（这正是跨内核对照要找的东西）：\n  {}",
            分岔.len(), 分岔.join("\n  "));
...
fn 真机那条读数在修前是分岔的() {
    ...
    assert_eq!(rust出口(0.56, 0.66, 0.56, 0.04), "band",
               "**修前 Rust 给 ignore（p <= lo），Python 给 band（p > lo - δ）**");
    // δ=0 时才回到修前那种裸比较
    assert_eq!(rust出口(0.56, 0.66, 0.56, 0.0), "ignore", "δ=0 时两种写法才一致");
}
