...
fn spend(escalate: u64) -> Budget {
    Budget { calls: 4, cost: 0.0, depth: Some(32), escalate: Some(escalate) }
}

fn calibrations() -> CalibStore {
    let mut calib = CalibStore::new();
    calib.put(CALIB, 0.8, 0.2, 100, "上岗").expect("校准记录合法");
    calib
}
...
fn client() -> FixedClient {
    let mut client = FixedClient::new();
    let state = State::new(vec![Mat::literal(material())], vec![], vec![], vec![], false);
    client.fix_ask(&state, &Question::new(Op::Test, ASK, CALIB, vec![]), Some(Answer::Noul(0.9)));
    client.observe(&state, &Question::new(Op::Test, LITERAL, CALIB, vec![]), Answer::Noul(0.95));
    client
}
...
fn duty_program(arm: Expr, escalate: u64) -> Program {
    build(
        ...
        vec![
            ...
            bind("s", call("state", vec![call("mat", vec![rec(vec![("x", int(1))])])])),
        ],
        call(
            "handle",
            vec![
...
fn go(program: &Program) -> Result<jpp_core::Outcome, Error> {
    let mut c = client();
    let mut ledger = Ledger::new();
    run(program, &mut c, &calibrations(), &ActionRegistry::new(), &mut ledger)
}
...
fn 忽略未决责任的臂是错() {
    ...
    let Err(Error::Runtime(e)) = go(&program) else { panic!("应当报 J-05") };
    assert_eq!(e.rule.as_deref(), Some("J-05"));
    assert_eq!(e.span, at, "出错位置指着那条臂");
    assert!(e.message.contains("escalate"), "报文要列出合法去向：{}", e.message);
}

/// 只读原因不算处理：`unsure_cause(u)` 是读展示快照，不转移责任。
#[test]
fn 只读原因不算处理() {
    let program = duty_program(lambda(&["u"], body(vec![], call("unsure_cause", vec![name("u")]))), 0);
    let Err(Error::Runtime(e)) = go(&program) else { panic!("应当报 J-05") };
    assert_eq!(e.rule.as_deref(), Some("J-05"));
}
...
fn 字面量臂静态就被拦() {
    ...
    let mut c = client();
    let mut ledger = Ledger::new();
    let e = run_unchecked(&program, &mut c, &calibrations(), &ActionRegistry::new(), &mut ledger).unwrap_err();
    assert_eq!(e.rule.as_deref(), Some("J-05"));
}
...
fn otherwise兜不住未决责任() {
    let program = build(
        ...
        call(
            "handle",
            vec![call("unsure", vec![text("材料不够")]), rec(vec![("otherwise", int(1))])],
        ),
    );
    let report = check(&program);
    let d = report.find("J-05").unwrap_or_else(|| panic!("应当报 J-05：\n{}", report.render()));
...
fn 包进返回值是合法去向() {
    let program = duty_program(
        lambda(&["u"], body(vec![], rec(vec![("待办", name("u")), ("下一步", text("补材料"))]))),
        0,
    );
...
fn escalate交给人是合法去向() {
    let program = duty_program(
        lambda(&["u"], body(vec![], call("escalate", vec![name("u"), name("s"), name("q")]))),
        1,
    );
    assert!(check(&program).is_ok(), "{}", check(&program).render());
    let outcome = go(&program).unwrap_or_else(|e| panic!("{}", e.render()));
    assert_eq!(outcome.value_json()["exit"], json!("act"), "人答了，换来一个已决出口");
    ...
    assert!(check(&no_budget).find("J-07").is_some(), "{}", check(&no_budget).render());
}
                ...
                call(
                    ...
                    vec![
                        ...
                        rec(vec![
                            ("act", text("收")),
                            ("ignore", text("弃")),
                            ("unsure", lambda(&["v"], body(vec![], call("unsure", vec![name("v")])))),
                        ]),
                    ],
                ),
            ),
...
fn 显式丢弃合法但留痕() {
    let program = duty_program(
        lambda(&["u"], body(vec![discard(call("consume", vec![name("u"), text("drop")]))], text("放着不管"))),
        0,
    );
    let outcome = go(&program).unwrap_or_else(|e| panic!("{}", e.render()));
    assert_eq!(outcome.value_json(), json!("放着不管"));
    assert_eq!(outcome.trace.warnings.len(), 1);
    assert!(outcome.trace.warnings[0].starts_with("W-drop-vs-escalate"), "{:?}", outcome.trace.warnings);
}

/// 责任不能换个壳就消失：变成材料也不行。
#[test]
fn 责任不能变成材料() {
    let program = duty_program(lambda(&["u"], body(vec![], call("mat", vec![name("u")]))), 0);
    let Err(Error::Runtime(e)) = go(&program) else { panic!("应当报 J-05") };
    assert_eq!(e.rule.as_deref(), Some("J-05"));
...
fn 捕获责任的方法不能交给map() {
    ...
    let apply_all = S::Function {
        ...
        function: Function {
            parameters: vec![
                Parameter { name: "f".into(), annotation: Some(linear), span: sp() },
                Parameter { name: "xs".into(), annotation: None, span: sp() },
            ],
            result_type: None,
            effects: None,
...
fn 方法类型带着效应行走() {
    ...
    let driver = S::Function {
        ...
        function: Function {
            parameters: vec![
                Parameter { name: "method".into(), annotation: Some(judging), span: sp() },
                Parameter { name: "m".into(), annotation: None, span: sp() },
            ],
            result_type: None,
    ...
    let opaque = S::Function {
        ...
        function: Function {
            parameters: vec![
                Parameter {
                    name: "method".into(),
                    annotation: Some(Type::Function(vec![Type::Named("Mat".into())], Box::new(Type::Named("Record".into())))),
                    span: sp(),
                },
                Parameter { name: "m".into(), annotation: None, span: sp() },
            ],
    ...
    assert!(check(&loose).find("E-effect").is_none(), "{}", check(&loose).render());
}
