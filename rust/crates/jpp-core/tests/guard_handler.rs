    ...
    fn judge(&mut self, s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
        *self.1.borrow_mut() += 1;
        ...
        let answers = qs.iter().map(|q| match q.op {
            ...
            jpp_core::value::Op::Test => Answer::Noul(self.0),
        }).collect();
...
"#, 0.5).expect_err("unsure 臂里的不可逆 do 不该自由执行");
    assert!(e.contains("J-08"), "{e}");
}
...
"#);
    assert!(r.is_ok(), "**合取语义下这里放行**——形状钉住，不是意外：{r:?}");
}
...
#[test]
fn 证书要说出自己只界定哪一侧() {
    use jpp_core::effects::{LiteralMode, Sample};
    ...
    // **这件事要是数据，不是一行注释**
    assert!(cert.bounded_side.contains("p ≥ hi"), "要说出界的是哪一侧：{}", cert.bounded_side);
    assert!(cert.bounded_side.contains("lo"), "并且要说出另一侧的实况：{}", cert.bounded_side);
    assert_eq!(c.get("k").lo, 0.0);
    println!("证书的单侧声明：{}", cert.bounded_side);
}

#[allow(dead_code)]
fn _用到(t: Taint) -> bool { t == Taint::Trusted }
