...
use jpp_core::effects::{Client, JevClient};
...
fn 三候选状态() -> Rc<State> {
    Rc::new(State::new(
        ...
        vec![],
        vec![],
        vec![Mat::literal(json!("甲")), Mat::literal(json!("乙")), Mat::literal(json!("丙"))],
        false,
    ))
}
...
fn select要发正逆两个置换并算众数占比() {
    ...
    let mut c = JevClient::with_transport(
        ...
        Box::new(move |body: &Json| {
            ...
            let crit = body["questions"]["q0"]["criteria"].as_object().expect("有候选表");
            let 甲在第几位 = crit.iter().find(|(_, v)| v.as_str() == Some("甲")).map(|(k, _)| k.clone()).expect("候选里有甲");
    ...
    assert_eq!(发过的.borrow().len(), 2, "select 要发**两个置换**（正序、逆序），实际发了 {} 次", 发过的.borrow().len());
    assert_eq!(r.mode_share.len(), 1, "一道题一个 mode_share");
    assert_eq!(r.mode_share[0], Some(1.0), "两次都选同一项：众数占比 2/2 = 1.0。实际 {:?}", r.mode_share[0]);

    // 两次请求的候选顺序要真的不同——否则「置换」是假的
    let 发过的 = 发过的.borrow();
    let 取候选 = |b: &Json| -> Vec<String> {
        b["questions"]["q0"]["criteria"].as_object().map(|m| m.values().map(|v| v.to_string()).collect()).unwrap_or_default()
    };
    assert_ne!(取候选(&发过的[0]), 取候选(&发过的[1]), "两次的候选顺序必须不同，否则不是置换");
}
...
fn 两次选了不同候选时众数占比小于一() {
    ...
    // **默认不发置换**（调用数 ×2 的钱不替作者花），要测它就显式开
    c.permute = true;
    let r = c.judge(&三候选状态(), &[&Question::new(Op::Select, "挑一个", "k", vec![])]).expect("跑得通");
    assert_eq!(r.mode_share[0], Some(0.5), "两次选了不同候选：众数占比 1/2 = 0.5。实际 {:?}", r.mode_share[0]);
}
...
fn 非select不发置换() {
    ...
    // **默认不发置换**（调用数 ×2 的钱不替作者花），要测它就显式开
    c.permute = true;
    let s = Rc::new(State::new(vec![Mat::literal(json!("对象"))], vec![], vec![], vec![], false));
    let r = c.judge(&s, &[&Question::new(Op::Test, "行吗", "k", vec![])]).expect("跑得通");
    assert_eq!(*次数.borrow(), 1, "noul 只发一次");
    assert_eq!(r.mode_share[0], None, "noul 上没有「置换」这回事，是 None 不是 0");
...
fn 费用与token照记() {
    let mut c = JevClient::with_transport(
        "jev-1.13.0",
        Box::new(|_b: &Json| Ok(json!({"answers": {"q0": {"noul": 0.5}}, "usage": {"input_tokens": 1000}}))),
    );
    // **默认不发置换**（调用数 ×2 的钱不替作者花），要测它就显式开
    c.permute = true;
...
fn 置换一致率与python的八条对得上() {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../foundation/experiments/raw/e_cal/readings.jsonl");
    ...
    for d in &原生 {
        ...
        let 原始最大 = probs.iter().max_by(|a, b| a.1.as_f64().unwrap_or(0.0).partial_cmp(&b.1.as_f64().unwrap_or(0.0)).unwrap()).map(|(kk, _)| kk.clone()).unwrap();
        let 赢家内容 = format!("候选{原始最大}");
        let mut c = JevClient::with_transport(
            "jev-1.13.0",
            Box::new(move |body: &Json| {
                let crit = body["questions"]["q0"]["criteria"].as_object().expect("有候选表");
                let 赢家位 = crit.iter().find(|(_, v)| v.as_str() == Some(赢家内容.as_str())).map(|(kk, _)| kk.clone()).expect("赢家在候选里");
                let mut out = serde_json::Map::new();
                for kk in crit.keys() {
                    out.insert(kk.clone(), json!(if *kk == 赢家位 { 0.9 } else { 0.1 / (crit.len() - 1) as f64 }));
                }
    ...
    // **默认不发置换**（调用数 ×2 的钱不替作者花），要测它就显式开
    c.permute = true;
        let over: Vec<Mat> = (0..k).map(|i| Mat::literal(json!(format!("候选{i}")))).collect();
        let s = Rc::new(State::new(vec![Mat::literal(json!("对象"))], vec![], vec![], over, false));
        let r = c.judge(&s, &[&Question::new(Op::Select, "挑一个", "e_cal.choice", vec![])]).expect("跑得通");
    ...
    let python一致数 = 原生.iter().filter(|d| d["ans"]["mode_share"].as_f64() == Some(1.0)).count();
