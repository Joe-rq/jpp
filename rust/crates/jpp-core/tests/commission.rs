...
use jpp_core::conformal::{certify, Certificate};
use jpp_core::effects::{CalibStore, LiteralMode, Refusal, Sample};
use serde_json::Value as Json;

fn 取(t: &str) -> Vec<(f64, bool, String)> {
    let j: Json = serde_json::from_str(include_str!("ecal_fixture.json")).unwrap();
    j[t].as_array().unwrap().iter()
...
fn 积累的证据要上岗必须过证书门() {
    ...
    assert_eq!((rec.status.as_str(), rec.n, rec.samples.len()), ("待真值", 73, 73));

    // **修前这一行 ok，修后必须错**
    let e = store.put("e_cal.noul", 0.78, 0.22, 73, "上岗").expect_err("手写的线不该让积累的证据上岗");
    assert!(e.contains("commission"), "错误要指出正门在哪：{e}");

    // 正门：跑证书。这批数据上 α=0.10 认证不过
    let 拒 = store.commission("e_cal.noul", 0.10, 0.10, "条").expect_err("α=0.10 在这批数据上过不了");
    match &拒 {
        Refusal::认证不过(Certificate::Refused { best_ucb, n_needed, .. }) => {
            println!("证书拒绝：best_ucb={best_ucb:.3} n_needed={n_needed}");
            assert_eq!(*n_needed, 22, "**要能说出还差多少条**——它是唯一告诉作者「这条路有终点」的东西");
    ...
    assert_eq!(store.commission("e_cal.noul", 0.0, 0.10, "条").expect_err("α=0 越界").n_needed(), None);
    // 拒绝不改记录：留在待真值
    assert_eq!(store.get("e_cal.noul").status, "待真值", "认证不过就留在待真值——**不阻塞，但也不放行**");
}
...
fn 过了证书就上岗且证书存进记录() {
    ...
    let cert = store.commission("k", 0.45, 0.10, "条").expect("α=0.45 认得动");
    let rec = store.get("k");
    assert_eq!(rec.status, "上岗");
    assert_eq!(rec.hi, cert.hi, "线由证书定，不由调用方写");

    assert_eq!(rec.lo, 0.0, "**证书只管放行那一侧**：弃权那一侧没有凭据，就不放行任何 Ignore");
    ...
    println!("认证线 hi={:.3} 放行 {}/73 错 {} 上界 {:.3}", rec.hi, c.n_accepted, c.n_errors, c.ucb);
}
...
#[test]
fn 线相同而证书不同时哈希要不同() {
    use jpp_core::effects::calib_hash;
    ...
    手填.put("k", 线.0, 线.1, 73, "上岗").expect("宿主手填这条路仍然通");
    assert_eq!((手填.get("k").hi, 手填.get("k").lo), 线, "线一模一样");
    assert_ne!(calib_hash(&宽), calib_hash(&手填), "**线相同、凭据不同，哈希必须不同**");
}
...
fn 簇的单位必须声明且不许推断() {
    ...
    assert!(!matches!(r, Err(Refusal::跑不成(_))), "有簇 id 时不该报「跑不成」：{r:?}");
...
fn 簇级比条级严() {
    ...
    for seed in 0..200u64 {
        if !certify(&jpp_core::conformal::cluster_subsample(&取("score"), seed), 0.30, 0.10).is_refused() {
            簇级有解 += 1;
        }
    }
    println!("score α=0.30：条级(n=55) 有解={条级有解}；簇级 200 次重采样有解 {簇级有解}/200");
    assert!(条级有解, "条级有解");
    assert_eq!(簇级有解, 0, "**簇级一次也不成立**——55 条读数不是 55 次独立观察");
}
...
fn 宿主手填不需要证书() {
    ...
    c.put("手填", 0.8, 0.2, 200, "上岗").expect("**这条路不该被证书门挡住**");
    assert_eq!(c.get("手填").status, "上岗");
    assert_eq!(c.get("手填").provenance(), jpp_core::effects::Provenance::宿主手填);
    assert!(c.get("手填").certs.is_empty(), "手填的线没有证书——而这一点本身是可查的");
}
