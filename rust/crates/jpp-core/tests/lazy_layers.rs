...
impl Client for 记账客户端 {
    ...
    fn calls(&self) -> u64 {
        self.每次题数.len() as u64
    }
}

fn 跑_带开关(src: &str, p: f64, passes: jpp_core::interp::Passes) -> (jpp_core::Outcome, Vec<usize>) {
    ...
    let 题数 = client.每次题数.clone();
    (out, 题数)
}

fn 跑(src: &str, p: f64) -> (jpp_core::Outcome, Vec<usize>) {
    let program = lower(&parse(src).expect("解析")).expect("lower");
    let mut calib = CalibStore::new();
    calib.put("k", 0.65, 0.35, 100, "上岗").unwrap();
    let mut client = 记账客户端 { p, 每次题数: vec![] };
...
"#;
    ...
    assert_eq!(out.value_json(), serde_json::json!({"甲": "act", "乙": "act"}), "两道题都该判出来");
    assert_eq!(
        每次题数,
        vec![2],
...
"#;
    let (out, 每次题数) = 跑(src, 0.9);
    assert_eq!(每次题数, vec![1, 1], "第二道题的状态依赖第一道的出口，必须分两层、各一次调用");
...
"#;
    ...
    assert_eq!(每次题数.len(), 3, "三个状态三次调用，不是六次：每次问两题。实际 {每次题数:?}");
    assert!(每次题数.iter().all(|n| *n == 2), "每次调用问两道题：{每次题数:?}");
    println!("省钱：即时执行 6 次调用 → 惰性 + 融合 {} 次，省 {} 次", out.cost.calls, 6 - out.cost.calls);
}
...
fn 关掉融合就逐题发_开关量得出成本差() {
    use jpp_core::interp::{Interp, Passes};
...
"#;
    ...
    let 跑一次 = |passes: Passes| {
        ...
        it.passes = passes;
        let out = it.run(&program).unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()));
        (out.cost.calls, client.每次题数.clone())
    };
    ...
    let 基 = Passes { vectorize: false, ..Passes::default() };
    let (开, 开题数) = 跑一次(基.clone());
    let (关, 关题数) = 跑一次(Passes { fuse: false, ..基 });

    assert_eq!(开, 3, "融合开：三个状态三次调用，每次两题。实际每次题数 {开题数:?}");
    assert!(开题数.iter().all(|n| *n == 2), "{开题数:?}");
    assert_eq!(关, 6, "融合关：逐题发，六次调用。实际每次题数 {关题数:?}");
    assert!(关题数.iter().all(|n| *n == 1), "关掉融合后每次只该问一道题：{关题数:?}");
    println!("融合的账：开 {开} 次调用 → 关 {关} 次，关掉成本涨 {:.0}%", (关 as f64 / 开 as f64 - 1.0) * 100.0);
}
...
fn 未落地的pass开关开着也不谎称在工作() {
    use jpp_core::interp::Passes;
    let 全开 = Passes { lift: true, fuse: true, fission: true, lower: true, schedule: true, plan: true, ledger: true, speculate: true, vectorize: true };
    ...
    for name in ["fission", "lower", "schedule", "plan"] {
        assert!(!全开.enabled(name), "{name} 还没落地，开关开着也不该说自己在工作");
    }
...
"#;
    ...
    assert_eq!(out.cost.calls, 2, "两个不同状态仍是两次调用——lift 省的是层数不是调用数");
    assert_eq!(每次题数, vec![1, 1]);
    assert_eq!(out.layers.len(), 1, "互不依赖，该在同一层发出；实际分了 {} 层：{:?}", out.layers.len(), out.layers);
}
...
"#;
    let (out, 每次题数) = 跑(src, 0.9);
    assert_eq!(out.value_json(), serde_json::json!({"甲": "act", "乙": "act"}));
    assert_eq!(每次题数, vec![2], "同状态、互不依赖：该随首个站点一起发，fuse 再合成一次调用。实际 {每次题数:?}");
...
{结果: 结果}
"#;
    let (out, 每次题数) = 跑_带开关(src, 0.9, Passes { speculate: false, ..Passes::default() });
    assert_eq!(out.value_json(), serde_json::json!({"结果": "act"}), "p=0.9 过上线，走 act 那边");
    assert_eq!(每次题数, vec![1, 1], "两次调用：lift 不许把分支里那个提到分支外");
    assert!(out.layers.len() >= 2, "lift 不跨分支，所以分层，实际 {:?}", out.layers);
}
...
"#;
    ...
    let 跑一次 = |passes: Passes| {
        ...
        it.passes = passes;
        let out = it.run(&program).unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()));
        (out.cost.calls, out.layers.len(), client.每次题数.clone())
    };

    let (开调用, 开层, 开题数) = 跑一次(Passes::default());
    let (关调用, 关层, 关题数) = 跑一次(Passes { lift: false, ..Passes::default() });

    assert_eq!((开调用, 开层), (1, 1), "lift 开：三道题随首个站点一起发，fuse 合成一次调用。实际每次题数 {开题数:?}");
    assert_eq!((关调用, 关层), (3, 3), "lift 关：各自等自己那次 cut，三层三次调用。实际 {关题数:?}");
