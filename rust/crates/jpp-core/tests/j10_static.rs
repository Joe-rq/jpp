...
fn 上岗记录(key: &str, unsure_rate: f64) -> (String, f64) { (key.to_string(), unsure_rate) }
...
fn 联合界超了就在跑之前报() {
    ...
    // Σuᵢ = 0.7
    let 松 = jpp_core::check_with_calib(&带unsure预算(两题, Some(0.8)), &c);
    assert!(松.find("J-10").is_none(), "0.7 ≤ 0.8，不该报：{}", 松.render());

    let 紧 = jpp_core::check_with_calib(&带unsure预算(两题, Some(0.5)), &c);
    let d = 紧.find("J-10").unwrap_or_else(|| panic!("0.7 > 0.5，该报：{}", 紧.render()));
    println!("{}", d.render());
    assert!(d.message.contains("0.7000"), "要把那个和说出来：{}", d.message);
...
fn 不设这一格就不报() {
    ...
    let r = jpp_core::check_with_calib(&带unsure预算(两题, None), &c);
    assert!(r.find("J-10").is_none(), "没设上限就没有「超」这回事：{}", r.render());
}
...
fn 未知的键按一计并且说出来() {
    ...
    let r = jpp_core::check_with_calib(&带unsure预算(两题, Some(0.5)), &c);
    let d = r.find("J-10").expect("两个未知键 → Σ = 2.0 > 0.5");
    assert!(d.message.contains("2.0000"), "{}", d.message);
    assert!(d.message.contains("2 个站点没有可用的 unsure_rate"), "**要说出这个和是怎么来的**：{}", d.message);
}
...
fn 报在任何模型调用之前() {
    ...
    // **检查器拿不到 Client**（签名里没有），所以「报在调用之前」是结构保证不是巧合。
    // 这里要测的是那个结构的**后果**：同一个程序，只检查 → 有诊断、零调用；跑 → 钱花了。
    let r = jpp_core::check_with_calib(&p, &c);
    assert!(r.find("J-10").is_some(), "只检查就报得出来");

    let mut 桩 = 数桩(RefCell::new(0));
    let mut l = jpp_core::ledger::Ledger::new();
    let out = jpp_core::run(&p, &mut 桩, &c, &jpp_core::interp::ActionRegistry::new(), &mut l).expect("跑得完");
    assert!(桩.calls() > 0, "前提：跑起来真的会发调用");
    assert!(
...
"#;
    ...
    let r = jpp_core::check_with_calib(&带unsure预算(src, Some(0.5)), &c);
    let d = r.find("J-10").unwrap_or_else(|| panic!("0.9 > 0.5 该报：{}", r.render()));
    println!("{}", d.render());
    assert!(
        d.message.contains("不是上界"),
        "**站点在函数体里，这个和不是上界，诊断必须自己说出来**：{}", d.message
    );
}
...
fn 没设这一格时两个入口给一样的报告() {
    let c = 记录本(&[上岗记录("ka", 0.9), 上岗记录("kb", 0.9)]);
    for src in [两题, "budget {calls: 1, cost: 1};\n1", "budget {calls: 1, cost: 1};\nask(state(mat(\"m\")), test(\"t\", \"ka\"))"] {
        let p = 带unsure预算(src, None);
        let 经记录 = jpp_core::check_with_calib(&p, &c);
        let 经档案 = jpp_core::check_with_profile(&p, &c.profile);
        assert_eq!(经记录.render(), 经档案.render(), "两个入口的报告要一样：\n{src}");
    }
}
