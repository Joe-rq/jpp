...
fn retag(source: &str, from: &str, to: &str) -> String {
    assert_eq!(source.matches(from).count(), 1, "「{from}」在样例里应当恰好出现一次");
    source.replace(from, to)
}

fn effect_error<'a>(report: &'a Report, what: &str) -> &'a jpp_core::Diagnostic {
    report.find("E-effect").unwrap_or_else(|| panic!("{what} 应当报 E-effect，实际诊断：\n{}", report.render()))
}
...
fn solve_的错标注在调用点被拦下() {
    ...
    assert!(d.message.contains("judge"), "报文要点出少了哪个效应：{}", d.message);
...
fn advance_的错标注在定义处被拦下() {
    ...
    let missing_do = retag(&source, "fn advance(s: Record, strategy) -> Record !{judge, do}", "fn advance(s: Record, strategy) -> Record !{judge}");
    ...
    let missing_judge = retag(&source, "fn advance(s: Record, strategy) -> Record !{judge, do}", "fn advance(s: Record, strategy) -> Record !{do}");
...
fn probe_的错标注被拦下() {
    ...
    let wrong = retag(BISECT, "fn probe(lo, hi, make_q) -> Record !{judge}", "fn probe(lo, hi, make_q) -> Record !{}");
    let report = check_source("bisect(probe 标成纯的)", &wrong);
    assert!(effect_error(&report, "probe 标成 !{}").message.contains("judge"));
}
...
fn search_的错标注被拦下_效应来自嵌套调用点() {
    let wrong = retag(BISECT, "fn search(start, make_q) -> Record !{judge}", "fn search(start, make_q) -> Record !{}");
    let report = check_source("bisect(search 标成纯的)", &wrong);
    assert!(effect_error(&report, "search 标成 !{}").message.contains("judge"));
}
...
fn supplement_的错标注被拦下() {
    ...
    let missing_do = retag(PARTIAL, "fn supplement(acc, und, checks) -> Record !{judge, do}", "fn supplement(acc, und, checks) -> Record !{judge}");
    let report = check_source("partial(supplement 漏标 do)", &missing_do);
    assert!(effect_error(&report, "supplement 漏标 do").message.contains("do"));

    let missing_judge = retag(PARTIAL, "fn supplement(acc, und, checks) -> Record !{judge, do}", "fn supplement(acc, und, checks) -> Record !{do}");
    let report = check_source("partial(supplement 漏标 judge)", &missing_judge);
    assert!(effect_error(&report, "supplement 漏标 judge").message.contains("judge"));
}
...
#[test]
fn grade_的错标注被拦下() {
    let wrong = retag(PARTIAL, "fn grade(acc, und, checks) -> Record !{judge}", "fn grade(acc, und, checks) -> Record !{}");
    let report = check_source("partial(grade 标成纯的)", &wrong);
    assert!(effect_error(&report, "grade 标成 !{}").message.contains("judge"));
}
...
"#;
    assert_clean("只创建、只存起方法值", &check_source("创建不等于执行", source));
}
...
"#;
    ...
    assert!(report.find("W-effect").is_none(), "judge 确实从 peek 那个调用点来，不该提示「标了却看不到」：\n{}", report.render());

    // 反过来：标成纯的就该被拦下
    let wrong = source.replace("fn apply(m, f) -> Record !{judge}", "fn apply(m, f) -> Record !{}");
    let report = check_source("并集（apply 标成纯的）", &wrong);
    assert!(effect_error(&report, "apply 标成 !{} 而有一个调用点传了 peek").message.contains("judge"));
}
...
{{a: f(peek).ok}}"
    ...
    assert!(effect_error(&report, "没有遮蔽时 f 调的就是那个会 judge 的形参").message.contains("judge"));
}
...
{{a: pure_user(mat({{x: 1}})).ok, b: judgy_user(mat({{x: 2}})).ok}}"
    ...
    assert_clean("传纯方法的那个调用点不该背 judge", &check_source("多态", &src));

    // 反过来：真的传了会 judge 的方法，标成纯的就得被拦下——证明上面那条不是把检查关掉换来的
    let wrong = retag(&src, "fn judgy_user(m) -> Record !{judge}", "fn judgy_user(m) -> Record !{}");
    let report = check_source("多态(judgy_user 标成纯的)", &wrong);
    let d = effect_error(&report, "judgy_user 经 apply 调用了 peek");
    assert!(d.message.starts_with("judgy_user"), "诊断要指着出错的那个函数：{}", d.message);
    assert!(d.message.contains("judge"), "{}", d.message);
    ...
    assert!(effect_error(&report, "apply 标成 !{} 而有调用点传 peek").message.starts_with("apply"));
}
...
{{a: use_maker(mat({{x: 1}})).ok}}"
    ...
    let wrong = retag(&src, "fn use_maker(m) -> Record !{judge}", "fn use_maker(m) -> Record !{}");
    let report = check_source("返回方法(标成纯的)", &wrong);
    assert!(effect_error(&report, "调用了 maker 返回的方法").message.starts_with("use_maker"));
}
...
{{a: use_boxed(mat({{x: 1}})).ok}}"
    ...
    let wrong = retag(&src, "fn use_boxed(m) -> Record !{judge}", "fn use_boxed(m) -> Record !{}");
    let report = check_source("记录字段(标成纯的)", &wrong);
    assert!(effect_error(&report, "调用了记录字段上的方法").message.starts_with("use_boxed"));
}
...
{{a: len(walk([mat({{x: 1}})], peek))}}"
    ...
    let wrong = retag(&src, "fn walk(ms, f) -> List !{judge}", "fn walk(ms, f) -> List !{}");
    let report = check_source("递归(标成纯的)", &wrong);
    assert!(effect_error(&report, "递归函数的方法参数在顶层收到 peek").message.starts_with("walk"));
}
...
{{a: apply(mat({{x: 1}}), g).ok, b: apply(mat({{x: 2}}), plain).ok}}"
    ...
    assert!(report.find("W-effect").is_none(), "有认不出的调用点就不该提示「标了却看不到」：\n{}", report.render());
