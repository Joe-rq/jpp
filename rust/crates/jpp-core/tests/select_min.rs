...
use jpp_core::effects::{CalibStore, Client, JevClient};
...
/// 「置换」这回事；`Some(0.5)` 模拟**测了、正逆两序选了不同的候选**（K=2 上判据是二值的）。
struct 不发置换的客户端 {
    inner: JevClient,
    mode_share: Option<f64>,
}
impl Client for 不发置换的客户端 {
    fn model_id(&self) -> String {
        self.inner.model_id()
    }
    fn judge(&mut self, s: &State, qs: &[&Question]) -> Result<jpp_core::effects::JudgeResult, jpp_core::effects::EffectError> {
        let mut r = self.inner.judge(s, qs)?;
        // K-noul 路径上 mode_share 无从谈起：它是 None，不是 0 也不是 1
        r.mode_share = qs.iter().map(|_| self.mode_share).collect();
        Ok(r)
    }
    fn generate(&mut self, p: &str, c: &[Json], n: usize, r: u64) -> Result<jpp_core::effects::GenResult, jpp_core::effects::EffectError> {
        self.inner.generate(p, c, n, r)
    }
    fn ask(&mut self, s: &State, q: &Question) -> Result<Option<jpp_core::value::Answer>, jpp_core::effects::EffectError> {
        self.inner.ask(s, q)
    }
    fn calls(&self) -> u64 {
        self.inner.calls()
...
"#;
    ...
    let mut client = 不发置换的客户端 {
        inner: JevClient::with_transport("jev-1.13.0", Box::new(|_b: &Json| Ok(json!({"answers": {"q0": {"probabilities": {"c0": 0.9, "c1": 0.05, "c2": 0.05}}}})))),
        mode_share: None,
    };
    let mut ledger = Ledger::new();
    let out = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect("程序应当跑完");
    ...
    assert!(
        out.trace.warnings.iter().any(|w| w.starts_with("W-untested")),
        "「置换一致性没测过」要留痕（J-15 加宽后的措辞）。实际告警：{:?}",
        out.trace.warnings
    );
    let _ = (Mat::literal(json!(1)), Op::Test, Rc::new(RefCell::new(0)));
}

/// 跑一段 select 源码，`mode_share` 由调用方指定（`None` = 没测过，`Some(x)` = 测了）。
fn 跑select(src: &str, mode_share: Option<f64>) -> jpp_core::Outcome {
    let program = lower(&parse(src).expect("解析")).expect("lower");
    let mut calib = CalibStore::new();
    calib.put("k", 0.65, 0.35, 100, "上岗").unwrap();
    let mut client = 不发置换的客户端 {
    ...
    let mut ledger = Ledger::new();
    run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect("程序应当跑完")
}
...
fn 没测过那一位对handler可见() {
    ...
    println!("【没测】{v}");
    assert_eq!(v["原因"], json!("untested"), "路由键要是通用的 untested（五个载体共用一条路由），不是每个载体一条");
    assert_eq!(v["未测"], json!("permutation"), "handler 要能读出**是哪个量**没测——只进 trace 不算可见");
    assert_eq!(v["出口"], json!("unsure(untested:permutation)"), "审计面（程序自己带进返回值的 exit_kind）要带着那一位");
}
...
fn 测了不一致才是tie而且不带那一位() {
    ...
    let v = out.value_json();
    println!("【测了不一致】{v}");
    assert_eq!(v["原因"], json!("tie"), "测了、不一致 = tie（12:151 只把 tie 指派给这一种）");
    assert_eq!(v["未测"], json!(""), "测过了就不该亮 J-15 那一位");
    assert_eq!(v["出口"], json!("unsure(tie)"), "tie 的标签不该被那一位污染");
    assert!(
        !out.trace.warnings.iter().any(|w| w.starts_with("W-untested")),
        "测过了不该报 W-untested：{:?}",
        out.trace.warnings
...
"#;
    ...
    let mut client = 不发置换的客户端 {
        inner: JevClient::with_transport("jev-1.13.0", Box::new(|_b: &Json| Ok(json!({"answers": {"q0": {"noul": 0.9}}})))),
        mode_share: None,
    };
    let mut ledger = Ledger::new();
    let out = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect("程序应当跑完");
    ...
    assert_eq!(v["原因"], json!("cold"), "路由键不动：cold 是 §5 已有的路由（保守线 + 标记）");
    assert_eq!(v["未测"], json!("calib_line"), "线未测是 J-15 的同一位，只是载体不同");
    assert_eq!(v["出口"], json!("unsure(cold|untested:calib_line)"), "路由键在前，那一位挂后面");
    assert!(
        out.trace.warnings.iter().any(|w| w.starts_with("W-untested") && w.contains("calib_line")),
...
fn 测了一致且过线才给pick_并钉住这个夹具的p在线上() {
    ...
    let v = out.value_json();
    println!("【测了一致】{v}");
    assert_eq!(v["原因"], json!("pick"), "测了、一致、p ≥ hi → Pick；这也钉住了本夹具的 p 在线之上");
