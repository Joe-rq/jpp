...
use jpp_core::{lower, run, syntax::parse};
...
"#;
    ...
    let e = run(&program, &mut 桩, &store, &a, &mut Ledger::new()).expect_err("类线不放行");
...
"#;
    ...
    let o = run(&program, &mut 桩, &store, &a, &mut Ledger::new()).unwrap();
