...
use jpp_core::interp::{json_to_value, ActionRegistry, Interp, Passes};
...
impl Client for C {
    ...
    fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
        for q in qs {
            self.log.borrow_mut().push(format!("judge:{}", q.text));
        }
...
fn run(src: &str, page: Json) -> (Result<jpp_core::Outcome, String>, bool) {
...
fn b_不可信单字符串不污染含它的字面量_放行() {
    放行("B", prog(r#"mat("2026-09-27 会议纪要")"#), Json::String("7".into()));
}

#[test]
fn c_一位数不可信数值经text拼接_拦下() {
    拦下("C", prog(r#"mat("转账 " + text(拆了.amount) + " 元")"#), serde_json::json!({"amount": 7}));
}

#[test]
fn c2_四位数不可信数值经text拼接_拦下() {
    拦下("C2", prog(r#"mat("转账 " + text(拆了.amount) + " 元")"#), serde_json::json!({"amount": 1234}));
}
...
fn 计算值取字段返回叶子自身的位() {
    ...
    放行("取字段", prog(r#"mat({a: "常量", b: 拆了.amount}.a)"#), serde_json::json!({"amount": 7}));
    拦下("取字段2", prog(r#"mat({a: "常量", b: 拆了.amount}.b)"#), serde_json::json!({"amount": 7}));
}
...
"#;
    ...
    a.register("取外部数据", 0.0, true, TaintOut::Untrusted, |_| Ok(json_to_value(&serde_json::json!({"amount": 7}))));
...
#[test]
fn j08_报错说明材料由计算值构成() {
    let (o, _) = run(&prog(r#"mat("转账 " + text(拆了.amount) + " 元")"#), serde_json::json!({"amount": 7}));
    let e = o.err().unwrap();
