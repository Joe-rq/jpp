...
impl Client for 记账客户端 {
    ...
    fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
        self.每次题数.borrow_mut().push(qs.len());
        Ok(JudgeResult { answers: qs.iter().map(|_| Answer::Noul(self.p)).collect(), tokens: 0, cost: 0.0, mode_share: vec![], perms: vec![] })
    }
    ...
    fn calls(&self) -> u64 {
        self.每次题数.borrow().len() as u64
    }
}

fn 跑(src: &str, p: f64, passes: Passes) -> (jpp_core::Outcome, Vec<usize>) {
    ...
    let 题数 = client.每次题数.borrow().clone();
    (out, 题数)
}
...
{r: 结果}
"#;
    let (关, 关题数) = 跑(src, 0.9, Passes { speculate: false, ..Passes::default() });
    let (开, 开题数) = 跑(src, 0.9, Passes { speculate: true, ..Passes::default() });

    assert_eq!(开.value_json()["r"], serde_json::json!("甲act"), "结果不该被推测改变");
    assert_eq!(关.value_json()["r"], 开.value_json()["r"], "开关推测，程序的值必须一样");

    assert_eq!(关.layers.len(), 2, "关推测：条件一层、走到的那个分支体一层。实际 {:?}", 关.layers);
    assert_eq!(开.layers.len(), 1, "开推测：两侧的 judge 并入本层，一层发完。实际 {:?}", 开.layers);
    println!("推测的账：关 {} 层 / {} 次调用（{关题数:?}） → 开 {} 层 / {} 次调用（{开题数:?}）",
             关.layers.len(), 关.cost.calls, 开.layers.len(), 开.cost.calls);
}
...
{r: 结果}
"#;
    let (out, _) = 跑(src, 0.9, Passes { speculate: true, ..Passes::default() });
    assert!(
        out.trace.warnings.iter().any(|w| w.starts_with("W-spec-unused")),
        "走了甲那侧，乙那侧的推测没用上，要留痕：{:?}",
        out.trace.warnings
    );
}
...
"#;
    ...
    let (out, _) = 跑(src, 0.9, Passes { speculate: true, ..Passes::default() });
    assert_eq!(out.value_json()["r"], serde_json::json!("走了这边"), "没走的分支里的 do 不该被执行");
}
...
{r: 结果}
"#;
    let (开, _) = 跑(src, 0.9, Passes { speculate: true, ..Passes::default() });
    assert_eq!(开.value_json()["r"], serde_json::json!("甲"), "程序照常跑");
    assert_eq!(开.layers.len(), 2, "那个站点依赖分支内的名字，推不了，仍是两层：{:?}", 开.layers);
}
...
"#;
    ...
    assert_eq!(关.value_json()["r"], 开.value_json()["r"], "开关推测程序的值必须一样");
    println!(
    ...
    assert_eq!(开.cost.calls, 1, "共状态时推测应当是**零边际**：题免费、状态收费（P5）。实际 {} 次", 开.cost.calls);
}
...
{r: 结果}
"#;
    let (关, _) = 跑(src, 0.9, Passes { speculate: false, ..Passes::default() });
    let (开, 开题数) = 跑(src, 0.9, Passes { speculate: true, ..Passes::default() });
    ...
    assert!(
        开.trace.warnings.iter().any(|w| w.starts_with("W-spec-unused")),
        "白花的那次要留痕（12 §4 第 8 行）：{:?}",
        开.trace.warnings
    );
    println!(
...
"#;
    ...
    let (关, 关题数) = 跑(src, 0.9, Passes { speculate: false, ..Passes::default() });
    assert_eq!(关题数, vec![1, 1], "关推测时两道题各自一次调用，融合不跨层：{关题数:?}");
    assert_eq!(关.layers.len(), 2);

    // 同时关推测与融合：仍是两次，且每次一题（证明这两个开关各管各的）
    let (全关, 全关题数) = 跑(src, 0.9, Passes { speculate: false, fuse: false, ..Passes::default() });
