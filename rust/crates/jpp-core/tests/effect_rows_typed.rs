...
{{a: 1}}"
    ...
    assert!(effect_error(&report, "参数类型说会 judge").message.starts_with("apply"));

    let ok = src.replace("fn apply(m, f: Fn(Record) -!{judge}-> Record) -> Record !{}", "fn apply(m, f: Fn(Record) -!{judge}-> Record) -> Record !{judge}");
...
{{a: apply(mat({{x: 1}}), peek).ok}}"
    ...
    assert!(&src[d.span.start..d.span.end] == "peek", "位置要指着实参本身，实际指着「{}」", &src[d.span.start..d.span.end]);
    ...
    // 把参数类型放宽到 judge 也没事
    let widened = src.replace("f: Fn(Record) -!{}-> Record) -> Record !{}", "f: Fn(Record) -!{judge}-> Record) -> Record !{judge}");
    assert_clean("参数类型放宽后不该报", &check_source("参数类型放宽", &widened));
}
