...
fn 不认得的字段要报错而不是无声吞掉() {
    ...
    std::fs::write(dir.join("x.json"), r#"{"key":"x","hi":0.8,"lo":0.2,"n":5,"status":"上岗","新字段":1}"#).unwrap();
...
fn 装载认得结构化的label_source() {
    ...
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join("y.json"), r#"{"key":"y","hi":0.8,"lo":0.2,"n":5,"status":"上岗",
        "label_source":{"选择子集":{"判据":"两模型一致且至少一方高置信","与对错相关":0.527}}}"#).unwrap();
    let store = CalibStore::load(&dir).expect("读得进");
    match &store.get("y").label_source {
        LabelSource::选择子集 { 判据, 与对错相关 } => {
            assert_eq!(判据, "两模型一致且至少一方高置信");
            assert_eq!(*与对错相关, Some(0.527));
        }
        other => panic!("该是选择子集：{other:?}"),
    }
    assert!(!store.get("y").label_source.可疑(), "声明了判据也测了相关性，不可疑");
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn 混题型的样本不许认证() {
    ...
    for i in 0..20 {
        c.absorb("k", Sample {
            p: Some(0.3 + i as f64 * 0.03), label: Some(if i > 5 { 1 } else { 0 }), perms: 0,
    ...
    let s = format!("{e:?}");
    assert!(s.contains("noul") && s.contains("choice"), "要说清混了哪几种：{s}");
...
fn save写出来的load读得回去() {
    ...
    assert_eq!(back.get("k"), c.get("k"), "**逐字段相同**——往返不许丢任何一维");
    let _ = std::fs::remove_dir_all(&dir);
}
