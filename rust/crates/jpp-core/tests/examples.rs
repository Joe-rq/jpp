...
fn 样例源码一条错都不该报() {
    for file in ["composition.jpp", "adaptive.jpp", "partial.jpp"] {
        ...
        let errors: Vec<String> = report
            .errors()
            .iter()
            .map(|d| format!("  {} @ {}", d.render(), &source[d.span.start..d.span.end.min(source.len())]))
            .collect();
        assert!(errors.is_empty(), "{file} 被检查器误杀：\n{}", errors.join("\n"));
        if !report.warnings().is_empty() {
            println!("{file} 的提示：");
            for w in report.warnings() {
                println!("  {} @ {}", w.render(), &source[w.span.start..w.span.end.min(source.len())]);
            }
        }
    }
}
...
fn 高阶函数的错标注拦得住() {
    let cases: [(&str, &[(&str, &[&str])]); 2] = [
        (
            ...
            &[("solve", &["judge"]), ("step", &["judge"]), ("observe", &["judge"])],
        ),
        (
            "partial.jpp",
            &[("advance", &["judge", "do"]), ("validate", &["judge", "do"]), ("read", &["judge"]), ("inspect", &["do"])],
        ),
    ];
    for (file, targets) in cases {
        ...
        for (name, real) in targets {
            ...
            let d = report
                ...
                .unwrap_or_else(|| panic!("{file} 的 {name} 标成 !{{}} 应当报 E-effect：\n{}", report.render()));
            ...
            assert!(
                !report.diagnostics.iter().any(|d| d.rule == "E-effect" && d.message.starts_with(name)),
                "{file} 的 {name} 标对了却被报：\n{}",
                report.render()
            );
        }
