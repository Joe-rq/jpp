...
use jpp_core::conformal::cost_line;
...
fn 换代价矩阵线要动放行集合要变假放行率要跟着动() {
    ...
    for (fp, fn_) in [(1.0, 1.0), (10.0, 1.0), (1.0, 10.0)] {
        ...
        println!(
            ...
            c.line, c.n_accepted, c.n, c.n_false_accept, c.false_accept_rate.expect("放行非空")
        );
    ...
    assert!((行[0].2 - 0.300).abs() < 0.001, "**经验假放行率 0.300——代价对称时它就是这么烂**");
    ...
    assert!(行[1].1 < 行[0].1 && 行[2].1 > 行[0].1, "线动了，放行集合跟着动");
}
...
fn 候选表不去重与certify不同() {
    ...
    assert!(s.iter().any(|(p, _)| (*p - c.line).abs() < 1e-9) || c.line == 0.5 || c.line == 0.7,
            "候选表含样本点本身（重复值的「中点」就是它自己）：线={}", c.line);
}
...
fn 代价定线证书定能不能上岗() {
    ...
    store.set_label_set_id("e_cal.noul", "label-A").expect("写得进");
    ...
    match &r {
        Ok(cert) => {
            println!("代价线 {:.3} 拿到证书：放行 {} 错 {} 上界 {:.3}", cert.hi, cert.n_accepted, cert.n_errors, cert.ucb);
            assert!((cert.hi - 0.780).abs() < 0.001, "**线由代价矩阵定**，不由证书自己找");
            assert_eq!(store.get("e_cal.noul").status, "上岗");
            assert_eq!(cert.cost, Some((10.0, 1.0)), "证书要记下是哪个代价矩阵定的线");
        }
        Err(e) => println!("代价线认证不过（也是诚实结果）：{e:?}"),
    }

    // **J-16：同源就不许算代价线**
    store.set_label_set_id("e_cal.noul", "conf-B").expect("写得进");
    let e = store.commission_costed("e_cal.noul", 0.45, 0.10, "条", (10.0, 1.0));
    assert!(format!("{e:?}").contains("J-16"), "标注集与保形集同源要拦：{e:?}");
}
...
fn 代价线只对test题有定义() {
    ...
    assert!(format!("{e:?}").contains("test"), "非 test 题要说清代价线未定：{e:?}");
}
...
fn 参考值在jpp那侧也比不了大小() {
    ...
    impl Client for 桩 {
        ...
        fn generate(&mut self, _p: &str, _c: &[Json], _n: usize, _r: u64) -> Result<GenResult, EffectError> { Err(EffectError("x".into())) }
...
"#);
    ...
    // 判据那个数照常是数，比得了
    assert!(跑("b.union_bound >= 0.0").is_ok(), "union_bound 是判据，当然能比");
    // **参考值比不了、也算不了**
    assert!(跑("b.independent_any >= 0.0").is_err(), "**参考值不该能当判据比大小**");
    assert!(跑("b.independent_any + 0.0").is_err(), "**参考值不该能做算术**");
    // 但看得见、打得出
...
fn 同风险目标时取线更高的那张() {
    ...
    if 松.is_ok() && 严.is_ok() {
        assert_eq!(store.get("k").certs.len(), 2, "两个代价矩阵是两个格，不是覆盖");
        let 选中 = store.选中的证书("k").expect("有");
        assert_eq!(选中.cost, Some((10.0, 1.0)), "**同 α 取线更高的那张**：{:?}", 选中);
        assert!((store.get("k").hi - 0.780).abs() < 0.001);
    } else {
        println!("这批数据上 α=0.80 两张没都认住（松={:?} 严={:?}），这一格没被覆盖到", 松.is_ok(), 严.is_ok());
    }
}
