...
fn nested_method_composition_matches_direct_core_construction() {
    let source =
        jpp_core::syntax::parse(include_str!("../../../examples/composition.jpp")).unwrap();
    let lowered = jpp_core::lower(&source).unwrap();
    ...
    let typed = jpp_core::lower(&jpp_core::syntax::parse(&typed_source).unwrap()).unwrap();
