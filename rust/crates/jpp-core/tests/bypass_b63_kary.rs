...
use jpp_core::truth::{import_labels, ImportOptions, LabelRow};
...
fn 行(op: &str, n: usize) -> Vec<LabelRow> {
    (0..n)
        .map(|i| {
            let 高 = i % 3 != 0;
            let p = if 高 { 0.9 + (i % 10) as f64 * 0.009 } else { 0.3 + (i % 5) as f64 * 0.04 };
            let truth = i % 4;
...
fn k元划分经真值通道单侧认证上岗() {
    for op in ["select", "measure"] {
        ...
        let o = if op == "select" { jpp_core::value::Op::Select } else { jpp_core::value::Op::Measure };
        let d = store.delta_for(&r, o);
        assert!(r.hi + d > 0.5 && r.hi + d <= 0.9 + 1e-9, "{op}: hi {} δ {d}", r.hi);
        assert_eq!(r.lo, 0.0);
...
fn k元行格式错误拒收() {
    ...
    assert!(import_labels(&mut store, &[缺pick], &选项()).unwrap_err().contains("pick"));
    ...
    let a: LabelRow = serde_json::from_value(serde_json::json!({"key": "k", "item": "a", "p": 0.9, "label": true, "source": "computed"})).unwrap();
    let b: LabelRow = serde_json::from_value(serde_json::json!({"key": "k", "op": "select", "item": "b", "p": 0.9, "pick": 0, "label": 0, "source": "computed"})).unwrap();
    assert!(import_labels(&mut store, &[a, b], &选项()).unwrap_err().contains("混了"));
}
...
fn 桥按hi加delta出pick与at() {
    use jpp_value::bridge::{decide, CutInput};
    use jpp_value::value::{Answer, ExitKind};
    let 判 = |a: Answer, ms: Option<f64>| {
        decide(&CutInput { fail: None, absent: None, suspended: false, line: Some((0.6, 0.0)), cost_requested: false, answer: Some(a), delta: 0.15, mode_share: ms }).0
    };
    assert_eq!(判(Answer::Score(vec![0.1, 0.8, 0.1]), None), ExitKind::At(1));
    assert_eq!(判(Answer::Score(vec![0.3, 0.7, 0.0]), None), ExitKind::Unsure("band".into()));
    assert_eq!(判(Answer::Choice(vec![0.8, 0.2]), Some(1.0)), ExitKind::Pick(0));
    assert_eq!(判(Answer::Choice(vec![0.7, 0.3]), Some(1.0)), ExitKind::Unsure("band".into()));
    assert_eq!(判(Answer::Choice(vec![0.9, 0.1]), None), ExitKind::Unsure("untested".into()));
}
