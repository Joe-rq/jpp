...
impl Client for 记账 {
    ...
    fn calls(&self) -> u64 { self.0.borrow().len() as u64 }
}
...
fn 跑p(src: &str, passes: Passes) -> (u64, usize, Vec<usize>) {
    ...
    (o.cost.calls, o.layers.len(), c.0.borrow().clone())
}
...
{r: map([1,2,3], fn(i) {
    handle(cut(judge(s, test("行吗","k"))), {
        act: fn(){"a"}, ignore: fn(){"i"}, unsure: fn(u){consume(u,"drop");"u"}}) })}
"#;
    let 关 = Passes { vectorize: false, ..Passes::default() };
    for (名, src) in [("异状态·无 cut", 异状态无cut), ("同状态·无 cut", 同状态无cut),
                      ("异状态·带 cut", 异状态带cut), ("同状态·带 cut", 同状态带cut)] {
        let (c1, l1, q1) = 跑p(src, 关.clone());
        let (c2, l2, q2) = 跑(src);
        println!("{名}: 关 calls={c1} layers={l1} 题数={q1:?} → 开 calls={c2} layers={l2} 题数={q2:?}");
        assert!(c2 <= c1, "**向量化不许把调用数变多**：{名} {c1} → {c2}");
        assert!(l2 <= l1, "**也不许把层数变多**：{名} {l1} → {l2}");
        assert_eq!(q2.iter().sum::<usize>(), q1.iter().sum::<usize>().min(q2.iter().sum::<usize>()),
                   "**也不许把问的题数变多**：{名} {q1:?} → {q2:?}");
    }
    ...
    assert_eq!((关层, 开层), (3, 1), "**异状态·带 cut：三层压成一层**，这是 vectorize 唯一的余量");
}
...
"#;
    ...
    assert_eq!(calls, 1);
    assert_eq!(题数, vec![1], "**三轮问的是同一道题（体里没用到 i），只该问一次**：{题数:?}");
}
