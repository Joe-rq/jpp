...
use jpp_core::effects::{calib_hash, CalibStore, Client, EffectError, GenResult, JudgeResult};
...
fn 跑(calib: &CalibStore, ledger: &mut Ledger) -> jpp_core::Outcome {
    let program = lower(&parse(程序).expect("解析")).expect("lower");
    let mut c = 定值(0.7, RefCell::new(0));
    run(&program, &mut c, calib, &ActionRegistry::new(), ledger).expect("跑得完")
}
...
fn 换线重放要报w_header() {
    ...
    松.put("k", 0.60, 0.30, 50, "上岗").expect("写得进");
    let mut ledger = Ledger::new();
    let 首 = 跑(&松, &mut ledger);
    assert_eq!(首.value_json(), serde_json::json!("act"), "0.7 ≥ 0.60 → act");
    assert_eq!(首.cost.calls, 1);

    // 换一条更严的线，**带着同一份账本**重放
    let mut 严 = CalibStore::new();
    严.put("k", 0.80, 0.20, 50, "上岗").expect("写得进");
    ...
    // 而出口变了
    assert_eq!(再.value_json(), serde_json::json!("band"), "0.7 落进 0.80/0.20 的带内");
...
fn 线没变就不报() {
    ...
    calib.put("k", 0.60, 0.30, 50, "上岗").expect("写得进");
    let mut ledger = Ledger::new();
    跑(&calib, &mut ledger);
...
fn 空库有自己的哈希() {
    ...
    有.put("k", 0.6, 0.3, 50, "上岗").expect("写得进");
    assert_ne!(空, calib_hash(&有), "「一条记录都没有」与「有一条记录」是两种状态");
}
...
fn 哈希不随写入顺序变() {
    ...
    a.put("甲", 0.6, 0.3, 50, "上岗").unwrap();
    a.put("乙", 0.7, 0.2, 60, "上岗").unwrap();
    let mut b = CalibStore::new();
    b.put("乙", 0.7, 0.2, 60, "上岗").unwrap();
    b.put("甲", 0.6, 0.3, 50, "上岗").unwrap();
