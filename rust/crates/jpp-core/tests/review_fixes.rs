...
fn 上岗(c: &mut CalibStore, key: &str, phys: &str, mode_share: Option<f64>) {
    for i in 0..60 {
        ...
        c.absorb(key, 样本(p, u8::from(p > 0.55), phys, mode_share)).expect("折得进");
    }
...
1
"#;
    let r = jpp_core::check_with_calib(&带unsure预算(src, 0.5), &c);
    let d = r.find("J-10").unwrap_or_else(|| panic!("两条读数 Σ = 0.6 > 0.5，该报：{}", r.render()));
...
fn j10_告警随run带出() {
    ...
    let mut client = jpp_core::FixedClient::new();
    let mut l = jpp_core::ledger::Ledger::new();
    let out = jpp_core::run(&p, &mut client, &c, &jpp_core::interp::ActionRegistry::new(), &mut l).expect("跑得完");
    assert!(out.trace.warnings.first().is_some_and(|w| w.starts_with("J-10")), "{:?}", out.trace.warnings);

    let mut l = jpp_core::ledger::Ledger::new();
    let out = jpp_core::run_with_fits(&p, &mut client, &c, &jpp_core::interp::ActionRegistry::new(), &jpp_core::effects::FitRegistry::new(), &mut l).expect("跑得完");
    assert!(out.trace.warnings.iter().any(|w| w.starts_with("J-10")), "{:?}", out.trace.warnings);
}
...
/// 认证后换档案（δ 变了），这个率不再描述 `cut` 的出口：按未知（1）计，而且存盘装回后仍然成立。
#[test]
fn unsure率绑定认证时的delta() {
    ...
    assert_eq!(r.unsure_rate_delta, Some(c.profile.delta.0));
    ...
    assert_eq!(装回.get("k").unsure_rate_delta, r.unsure_rate_delta, "δ 随记录落盘");
    装回.profile.delta.0 += 0.05;
    assert_eq!(装回.usable_unsure_rate(&装回.get("k")), None, "δ 变了，率不可用");
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn 拆分认证与行序无关() {
    let rows: Vec<(f64, u8)> = (0..240)
        .map(|i| {
            ...
            // 中间带里掺一些错标，让分半真的影响结论
            let label = if (i % 11 == 0) && (0.3..0.7).contains(&p) { u8::from(p <= 0.5) } else { u8::from(p > 0.5) };
            (p, label)
        })
        .collect();
    let 认证 = |order: &[(f64, u8)]| {
    ...
    反序.reverse();
    assert_eq!(正, 认证(&反序));
    let mut 交错: Vec<(f64, u8)> = rows.iter().step_by(2).chain(rows.iter().skip(1).step_by(2)).cloned().collect();
    交错.rotate_left(17);
    assert_eq!(正, 认证(&交错));
}
...
fn 行(item: String, p: f64, label: bool, source: &str) -> LabelRow {
    serde_json::from_value(serde_json::json!({"key": "k", "item": item, "p": p, "label": label, "source": source})).unwrap()
}
...
fn 旧的待核模型标注不因新一批人工行而上岗() {
    ...
    let 模型: Vec<LabelRow> = (0..200)
        .map(|i| 行(format!("m{i}"), if i % 2 == 0 { 0.97 } else { 0.02 }, i % 2 == 0, "model:x"))
        .collect();
    ...
    let 人工: Vec<LabelRow> = (0..200)
        .map(|i| 行(format!("h{i}"), if i % 2 == 0 { 0.96 } else { 0.03 }, i % 2 == 0, "human"))
        .collect();
    let r = import_labels(&mut c, &人工, &导入选项()).unwrap();
    assert!(r[0].truth.gate.starts_with("待核"), "旧模型标注没有抽检，不能借新人工行上岗：{}", r[0].truth.gate);
    assert_ne!(c.get("k").status, "上岗");
    assert_eq!(c.get("k").truth.unwrap().sources.get("model:x"), Some(&200), "合并后的来源账写回记录");
    let _ = std::fs::remove_dir_all(&dir);
}
