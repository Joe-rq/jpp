...
use jpp_core::conformal::Certificate;
use jpp_core::effects::{CalibStore, Client, EffectError, GenResult, JudgeResult, LiteralMode, Provenance, Refusal, Sample};
...
fn 跑完程序之后人还能写线上岗() {
    ...
    let mut c = 定值(0.9, RefCell::new(0));
    let mut l = Ledger::new();
    let out = run(&program, &mut c, &calib, &ActionRegistry::new(), &mut l).expect("跑得完");
...
fn 没标注与认证不过说的不是同一句() {
    ...
    assert!(b.n_needed().is_some(), "**「还差多少条」只有这一种拒绝答得出**");
}
...
fn 收紧不需要凭据放宽才需要() {
    ...
    c.put("k", 0.99, 0.0, 30, "上岗").expect("**收紧不该被拦**——拦住它就是失败开放：旧线继续放行而人改不动");
    assert_eq!(c.get("k").hi, 0.99);
    assert_eq!(c.get("k").certs.len(), 1, "收紧不抹掉证书");

    // **放宽**（hi 更低 = 多放行）：要凭据
    let e = c.put("k", 0.10, 0.0, 30, "上岗").expect_err("放宽必须要凭据");
    assert!(e.contains("commission"), "{e}");
    assert_eq!(c.get("k").hi, 0.99, "被拦住时线不动");

    // **而且不许把键弄死**：拦住之后这个键仍然认得回来
    assert_ne!(c.get("k").status, "停岗", "拦住换线不等于把键停掉——那会造出新的永久锁");
    let _ = 认证线;
}
...
"#).expect("解析")).expect("lower");
        let mut c = 定值(0.9, RefCell::new(0));
        let mut l = Ledger::new();
        let o = run(&program, &mut c, calib, &ActionRegistry::new(), &mut l).expect("跑得完");
...
fn 两张不同风险目标的证书并存而不是覆盖() {
    ...
    // 再认一次，α 更松 → 线更低（更宽松）
    let 弱 = c.commission("k", 0.80, 0.10, "条").expect("更松的 α 当然也认得动");
...
fn 同一个风险目标重认是更新那一格() {
    ...
    c.commission("k", 0.45, 0.10, "条").unwrap();
    assert_eq!(c.get("k").certs.len(), 1, "同 (α, conf_delta, 簇单位) 是同一格");
    // 但不同簇单位是不同的格：**按条取和按簇取不是同一个测量**
    let _ = c.commission("k", 0.45, 0.10, "对象段");
}

#[allow(dead_code)]
fn _用到(c: &Certificate) -> bool { c.is_refused() }
