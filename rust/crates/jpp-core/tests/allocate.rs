...
use jpp_core::strength::{allocate, unsure_bound};
use jpp_core::value::{Answer, Op, Reading};
use serde_json::Value as Json;

fn oracle() -> Json {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/oracle/oracle.json");
    let text = std::fs::read_to_string(&path).unwrap_or_else(|e| panic!("读不到对照基准 {}：{e}（跑 tests/oracle/generate.py 生成）", path.display()));
    serde_json::from_str(&text).expect("对照基准是合法 JSON")
}

fn f(j: &Json, k: &str) -> f64 {
    j.get(k).and_then(|v| v.as_f64()).unwrap_or_else(|| panic!("基准里缺 {k}"))
}
...
fn store(case: &Json, profile: &Json) -> CalibStore {
    ...
    calib.profile.safety = (s[0].as_f64().unwrap(), s[1].as_f64().unwrap());
    let d = &profile["delta"];
    calib.profile.delta = (f(d, "noul"), f(d, "choice"), f(d, "score"));
    ...
    calib.put("k", f(case, "hi"), f(case, "lo"), n, status).expect("校准记录合法");
        ...
        .map(|(i, p)| {
            ...
            let (op, answer) = match phys {
                ...
                "score" => {
                    ...
                    (Op::Measure, Answer::Score((0..levels).map(|l| if l == 0 { p } else { rest }).collect()))
                }
            ...
            Rc::new(Reading {
                ...
                over_len: 0,
                scale: case["scale"].as_array().unwrap().iter().map(|x| x.as_str().unwrap().to_string()).collect(),
...
fn 与python内核选出同一批下标() {
    ...
    assert!(cases.len() >= 12, "基准至少该有十几条，实际 {}", cases.len());
    for case in cases {
...
fn 与python内核给出同一组界() {
    ...
    for case in o["cases"].as_array().unwrap() {
        ...
        let want = &case["bound"];
        assert_eq!(got.n as u64, want["n"].as_u64().unwrap(), "用例 {name} 的 n");
        assert_eq!(got.n_unknown as u64, want["n_unknown"].as_u64().unwrap(), "用例 {name} 的 n_unknown");
        // 两边都已经 round 到四位；比的是同一个数，不留 epsilon 的余地
        assert_eq!(got.union_bound, f(want, "union_bound"), "用例 {name} 的联合界");
        assert_eq!(got.independent_any.as_reference_only(), f(want, "independent_any"), "用例 {name} 的独立估计（只作参考值）");
    }
}
