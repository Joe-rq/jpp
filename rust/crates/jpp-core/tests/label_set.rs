...
fn 证书总是带着标注集指纹() {
    ...
    assert_eq!(cert.label_fp.len(), 16, "取 16 位十六进制，与 profile_hash 同口径");
}
...
fn 不同标注集是不同的证书格() {
    ...
    assert_ne!(ca.addr(), cb.addr(), "**指纹进地址**：两份标注集上的证书不许占同一格");
}
...
fn 声明与实际对不上要拦() {
    let mut c = CalibStore::new();
    装(&mut c, "k", 0.0);
    let 真指纹 = c.commission("k", 0.45, 0.10, "条").expect("认得动").label_fp;

    // 声明一个对得上的
    c.declare_label_set("k", "e-cal-v2", "foundation/runs/jv/e-cal/labels.csv", &真指纹).unwrap();
    assert!(c.commission("k", 0.40, 0.10, "条").is_ok(), "声明对得上，照常认证");

    // 声明一个对不上的 → 拦
    c.declare_label_set("k", "别的集", "somewhere/else.csv", "0000000000000000").unwrap();
    let e = c.commission("k", 0.35, 0.10, "条").expect_err("声明与实际对不上要拦");
    let s = format!("{e:?}");
    assert!(s.contains("指纹") && s.contains("别的集"), "要说清是哪一份对不上：{s}");
}
...
fn 没有数据的机器上仍然读得到线和凭据() {
    ...
    assert_eq!(无数据.选中的证书("k").unwrap().label_fp, 指纹, "凭据照常比得了");
    // 但重认证需要数据——**这本来就该需要数据**
    let e = 无数据.commission("k", 0.45, 0.10, "条").expect_err("没有数据就重认不了");
