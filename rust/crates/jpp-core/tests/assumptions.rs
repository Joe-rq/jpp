...
fn 档(field: Tri) -> Profile {
    ...
    match field {
        Tri::真 => { j["arithmetic_capable"] = serde_json::json!(true); }
        Tri::假 => { j["arithmetic_capable"] = serde_json::json!(false); }
        Tri::未测 => { j.as_object_mut().unwrap().remove("arithmetic_capable"); }
    }
    Profile::from_json(&j).expect("真档案加一个字段仍该读得动")
}

fn 判(src: &str, p: &Profile) -> jpp_core::check::Report {
...
fn 档案说模型会算术时j01的算术面降warn() {
    ...
    assert_eq!(d.severity, Severity::Warning, "档案说模型会算术 → 降 warn（12 §1.3）：{}", d.render());
    assert!(说会.is_ok(), "warn 不阻塞");
    assert!(d.message.contains("arithmetic_capable"), "要说出是哪条档案字段让它降的：{}", d.message);
}
...
fn 比较那一面不跟着降() {
    for t in [Tri::假, Tri::真, Tri::未测] {
        ...
        assert_eq!(d.severity, Severity::Error, "{t:?} 下比较仍是错：{}", d.render());
    }
}
...
fn 没档案与档案说成立区分得开() {
    ...
    assert!(无档.find("W-untested").is_some(), "没加载档案 = 这条假设没被任何测量支持，必须留痕：{}", 无档.render());
    assert!(有档.find("W-untested").is_none(), "档案明说了就不该报未测：{}", 有档.render());
}
...
fn 字段未测与没档案也分得开() {
    ...
    assert_eq!(未测.find("J-01").unwrap().severity, Severity::Error, "未测取保守项");
    let w = 未测.find("W-untested").expect("未测要报 W-untested（J-15）");
    assert!(w.message.contains("档案"), "要说清是「档案说未测」：{}", w.message);

    let 无档 = check(&lower(&parse(算术).expect("解析")).expect("lower"));
    let w2 = 无档.find("W-untested").expect("没档案也要报");
    assert_ne!(w.message, w2.message, "**「档案说未测」与「根本没档案」是两种不确定**");
}
...
fn 真档案里今天没有这个字段() {
    ...
    assert_eq!(Profile::from_json(&j).unwrap().arithmetic_capable, Tri::未测);
}
...
fn run这条路上档案也到得了检查器() {
    ...
    let 跑 = |p: Profile| {
        ...
        jpp_core::run(&program, &mut 桩, &store, &ActionRegistry::new(), &mut l).map(|_| ()).map_err(|e| format!("{e:?}"))
    };
    ...
    let r = 跑(档(Tri::真));
    let e = format!("{r:?}");
    assert!(!e.contains("Check("), "**降级之后不该再被静态挡住**，实得 {e}");
}
