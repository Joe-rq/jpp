...
use jpp_core::effects::{CalibStore, Profile};
...
fn 从档案读出的线与delta与python一致() {
    ...
    assert_eq!(p.safety, (s[0].as_f64().unwrap(), s[1].as_f64().unwrap()), "冷键的保守线要从档案 lines.safety_default 来");

    let d = &want["delta"];
    assert_eq!(
        p.delta,
        (d["noul"].as_f64().unwrap(), d["choice"].as_f64().unwrap(), d["score"].as_f64().unwrap()),
        "δ 要从档案 delta.<题式>.immediate.p99 来（choice 走 choice_prob_chosen）"
    );

    // 与代码兜底值确实不同——否则这条测试测不出东西
    let 兜底 = Profile::default();
    assert_ne!(p.safety, 兜底.safety, "档案值与兜底值本该不同，否则这条测试没有意义");
    assert_ne!(p.delta, 兜底.delta);
}
...
/// 这里选后者：`Profile::default()` 的 `hash` 是 `None`，账本头记成 `"<未加载档案>"`。
#[test]
fn 没加载档案时要留下痕迹而不是装作有() {
    let 兜底 = Profile::default();
    assert!(兜底.hash.is_none(), "兜底档案没有哈希——它不是一份档案，是代码常数");
    ...
    assert!(c.profile.hash.is_none(), "没接档案的 CalibStore 要能看出来没接");
}
...
fn 换档案重放会报w_header() {
    ...
    l.set_header(头(Some("档案甲")));
    l.set_header(头(None));
    assert!(l.header_warning.is_some(), "从有档案变成无档案，线与 δ 会变，必须告警");
}
...
fn mat要能数token且与python同一个估法() {
    ...
    let 期望 = (jpp_core::value::canon(&文本.content).chars().count() as f64 / 1.3) as usize + 1;
    assert_eq!(文本.tokens(), 期望, "token 估法要与 Python 的 int(len(canon)/1.3)+1 一致");

    let 长文 = Mat::literal(serde_json::json!("x".repeat(2600)));
    assert!(长文.tokens() > 1900, "2600 字符该估出 1900+ token，实际 {}", 长文.tokens());
}
...
fn 窗口上限从档案读() {
    ...
    assert!(p.text_window > 0, "对象槽内单段的可用窗口该从档案来，实际 {}", p.text_window);
    // Python `json_ctx_window()` 取 flip_frac_by_ctx_tokens 的最大档
    assert_eq!(p.json_ctx_window, 1800, "槽间窗口取 flip_frac_by_ctx_tokens 的最大档（~1800）");
}
...
#[test]
fn 行为摘要不随说明文字变() {
    use jpp_core::effects::{behavior_hash, profile_hash};
    use serde_json::json;
    ...
    assert_ne!(profile_hash(&原), profile_hash(&改了说明), "整份档案的哈希对说明敏感——这是它的定义，不动它");
    assert_eq!(behavior_hash(&原), behavior_hash(&改了说明), "**行为摘要不该随说明文字变**：行为一个字节没变");
    assert_ne!(behavior_hash(&原), behavior_hash(&改了线), "线变了行为摘要必须变");
}
...
/// 档案**缺窗口字段就报错**，不悄悄用兜底值——与线、δ 同一条（`12` §1）。
/// 这里原本是 `.unwrap_or(1000)` / `.unwrap_or(1800)`：同一个函数里对线严、对窗口松，
/// 而那两个兜底值正是**在替不确定说确定**。
#[test]
fn 档案缺窗口字段要报错() {
    use jpp_core::effects::Profile;
    use serde_json::json;
    ...
    let e = Profile::from_json(&缺窗口).expect_err("缺窗口字段该报错");
    assert!(e.contains("window"), "报文要说清缺的是哪个字段：{e}");

    // 兜底值本身合法——不合法的是 load 悄悄回退到它
    assert_eq!(Profile::default().text_window, 1000);
    assert!(Profile::default().hash.is_none(), "兜底不是档案，没有哈希");
}
