...
use jpp_core::effects::{Client, FixedClient};
...
fn 没给responses时静默挂起() {
    ...
    assert_eq!(c.ask(&状态("甲"), &q).expect("不该报错"), None, "**没人答就是没人答**");
}
...
fn 给了responses却没命中要出声() {
    let mut c = FixedClient::new();
    // 登记的是全角「？」
    c.fix_ask(&状态("甲"), &Question::new(Op::Test, "批不批？", "human", vec![]), Some(Answer::Noul(1.0)));
    // 程序问的是半角「?」
    let q = Question::new(Op::Test, "批不批?", "human", vec![]);
    let Err(e) = c.ask(&状态("甲"), &q) else { panic!("**给了却没命中，不该静默挂起**") };
    assert!(e.0.contains("批不批?"), "要说出程序问的是哪一道：{}", e.0);
    assert!(e.0.contains("responses"), "要说清是 responses 这一栏：{}", e.0);
    println!("{}", e.0);
}
...
fn 命中的照常答() {
    ...
    assert_eq!(c.ask(&状态("甲"), &q).expect("命中"), Some(Answer::Noul(1.0)));
}
...
fn 登记了但还没答的静默挂起() {
    ...
    assert_eq!(c.ask(&状态("甲"), &q).expect("不该报错"), None);
}
...
fn 打出来的json要能直接粘贴() {
    let mut c = FixedClient::new();
    let s = State::new(vec![Mat::literal(json!({"item": "flight"}))], vec![], vec![], vec![], false);
    let q = Question::new(Op::Measure, "多大把握", "k", vec!["低".into(), "高".into()]);
    let Err(e) = c.judge(&s, &[&q]) else { panic!("该未命中") };

    assert!(e.0.contains(r#""on":[{"item":"flight"}]"#), "**`on` 要是列表**，夹具只收列表：{}", e.0);
    assert!(e.0.contains(r#""op":"measure""#), "**`op` 要用夹具认的名字**（test/select/measure），不是内核的 noul/choice/score：{}", e.0);
...
"#);
    assert!(a.find("J-13").is_some() || a.find("W-seq-const").is_some(), "{}", a.render());
...
"#);
    assert!(b.find("J-13").is_some() || b.find("W-seq-const").is_some(),
            "**循环外绑的常量与字面量后果相同，不能静默**：{}", b.render());
...
"#);
    assert!(c.find("J-13").is_none() && c.find("W-seq-const").is_none(),
            "**`let 序号 = i + 1` 每轮都变，不该报**：{}", c.render());
}
