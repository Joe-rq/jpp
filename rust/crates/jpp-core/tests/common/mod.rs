...
//! `jpp_core::lower` 降到 IR）。每个节点拿一个唯一的 `Span`——测试要断言诊断落在哪个节点上，
...
pub use jpp_core::syntax::ast::{
    Block, Expr, ExprKind, Function, Parameter, Span, Statement, Type,
};
...
pub fn program(b: Option<Budget>, statements: Vec<Statement>, result: Expr) -> Program {
    ...
    let mut p = jpp_core::lower(&src).unwrap_or_else(|ds| panic!("降级失败：{ds:?}"));
    // 整个程序的位置单取一个（与步 12d 前的建造器同序：先程序体，后程序）
    let at = sp();
    p.span = jpp_core::Span {
        start: at.start,
        end: at.end,
    };
    p
...
pub fn with_effects(p: &Program, name: &str, effects: &[&str]) -> (Program, jpp_core::Span) {
    let mut out = p.clone();
    let mut at = None;
    for s in &mut out.body.statements {
        if let jpp_core::Stmt::Function {
            name: n,
            function,
            span,
        } = s
...
pub fn certified(calib: &mut jpp_core::effects::CalibStore, key: &str, hi: f64, lo: f64, n: u64) {
    ...
    let cert = jpp_core::effects::Cert {
        ...
        label_source: jpp_core::effects::LabelSource::全体,
    };
