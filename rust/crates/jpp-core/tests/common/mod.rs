...
pub fn rec(fields: Vec<(&str, Expr)>) -> Expr {
    Expr::new(ExprKind::Record(fields.into_iter().map(|(k, v)| (k.to_string(), v)).collect()), sp())
}
pub fn call(f: &str, args: Vec<Expr>) -> Expr {
    Expr::new(ExprKind::Call { function: Box::new(name(f)), arguments: args }, sp())
}
pub fn call_of(f: Expr, args: Vec<Expr>) -> Expr {
    Expr::new(ExprKind::Call { function: Box::new(f), arguments: args }, sp())
}
pub fn field(v: Expr, f: &str) -> Expr {
    Expr::new(ExprKind::Field { value: Box::new(v), field: f.to_string() }, sp())
}
pub fn index(v: Expr, i: Expr) -> Expr {
    Expr::new(ExprKind::Index { value: Box::new(v), index: Box::new(i) }, sp())
}
pub fn bin(op: &str, l: Expr, r: Expr) -> Expr {
    Expr::new(ExprKind::Binary { op: op.to_string(), left: Box::new(l), right: Box::new(r) }, sp())
}
pub fn not(v: Expr) -> Expr {
    Expr::new(ExprKind::Unary { op: "!".to_string(), value: Box::new(v) }, sp())
}
pub fn if_(c: Expr, yes: Expr, no: Expr) -> Expr {
    Expr::new(ExprKind::If { condition: Box::new(c), yes: body(vec![], yes), no: body(vec![], no) }, sp())
}

pub fn body(statements: Vec<Statement>, result: Expr) -> Block {
    Block { statements, result: Some(Box::new(result)), span: sp() }
}
...
pub fn bind(n: &str, v: Expr) -> Statement {
    Statement::Let { name: n.to_string(), annotation: None, value: v, span: sp() }
}

fn make(params: &[&str], effects: Option<&[&str]>, result_type: Option<Type>, b: Block) -> Function {
    Function {
        parameters: params.iter().map(|p| Parameter { name: p.to_string(), annotation: None, span: sp() }).collect(),
        result_type,
...
pub fn lambda_eff(params: &[&str], effects: &[&str], b: Block) -> Expr {
    Expr::new(ExprKind::Function(make(params, Some(effects), None, b)), sp())
}
/// 具名方法
pub fn func(n: &str, params: &[&str], b: Block) -> Statement {
    Statement::Function { name: n.to_string(), function: make(params, None, None, b), span: sp() }
}
pub fn func_eff(n: &str, params: &[&str], effects: &[&str], b: Block) -> Statement {
    Statement::Function { name: n.to_string(), function: make(params, Some(effects), None, b), span: sp() }
}
/// 带返回类型标注（J-05：标注含 Exit 才许把出口带出）
pub fn func_ret(n: &str, params: &[&str], effects: Option<&[&str]>, ret: Type, b: Block) -> Statement {
    Statement::Function { name: n.to_string(), function: make(params, effects, Some(ret), b), span: sp() }
}
...
pub fn program(b: Option<Budget>, statements: Vec<Statement>, result: Expr) -> Program {
    Program { budget: b, body: body(statements, result), span: sp() }
}

/// 把某个具名方法的效应标注换掉，用来验「错标注拦不拦得住」。返回改过的程序与那个方法的 Span。
pub fn with_effects(p: &Program, name: &str, effects: &[&str]) -> (Program, Span) {
    let mut out = p.clone();
    let mut at = None;
    for s in &mut out.body.statements {
        if let Statement::Function { name: n, function, span } = s {
            if n == name {
    ...
    (out, at.unwrap_or_else(|| panic!("程序里没有具名方法 {name}")))
}
