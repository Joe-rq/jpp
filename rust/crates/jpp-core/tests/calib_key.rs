...
fn 不同literal_mode是不同的校准格() {
    ...
    c.put_moded("k", LiteralMode::CodeLiteral, 0.70, 0.30, 100, "上岗").expect("写得进");
    c.put_moded("k", LiteralMode::DocSection, 0.55, 0.45, 80, "上岗").expect("写得进");

    let 代码 = c.get_moded("k", LiteralMode::CodeLiteral);
    let 文档 = c.get_moded("k", LiteralMode::DocSection);
    assert_eq!((代码.hi, 代码.lo, 代码.n), (0.70, 0.30, 100), "两格各是各的，第二个不该覆盖第一个");
    assert_eq!((文档.hi, 文档.lo, 文档.n), (0.55, 0.45, 80));

    // 没写过的那一档仍是冷的——**不继承别的模式的线**
    let 执行输出 = c.get_moded("k", LiteralMode::ExecOutput);
    assert_eq!(执行输出.status, "冷", "别的模式测过，不等于这一档测过：{:?}", 执行输出);
}
...
fn 不带模式的老接口仍可用() {
    let mut c = CalibStore::new();
    c.put("k", 0.65, 0.35, 100, "上岗").expect("写得进");
    assert_eq!(c.get("k").hi, 0.65);
    assert_eq!(c.get_moded("k", LiteralMode::default()).hi, 0.65, "老接口写的就是默认档");
}
