...
use jpp_core::strength::{uncertainty, unsure_bound};
...
fn unsure_bound逐例与python相同() {
    ...
    for c in gold["unsure_bound"].as_array().expect("用例") {
        let us: Vec<Option<f64>> = c["us"].as_array().unwrap().iter()
            .map(|v| if v.is_null() { None } else { Some(v.as_f64().unwrap()) }).collect();
        ...
        for (i, u) in us.iter().enumerate() {
            ...
            match u {
                ...
                Some(v) => { store.put(&k, 0.8, 0.2, 10, "上岗").unwrap(); store.set_unsure_rate(&k, *v).unwrap(); }
                None => { store.put(&k, 0.8, 0.2, 10, "上岗").unwrap(); }
            }
            rs.push(读数(&k, 0.5));
        }
        let b = unsure_bound(&store, &rs);
        let e = &c["out"];
        let 我 = (b.n as u64, (b.union_bound * 10000.0).round(), b.n_unknown as u64);
        let 它 = (e["n"].as_u64().unwrap(), (e["union_bound"].as_f64().unwrap() * 10000.0).round(), e["n_unknown"].as_u64().unwrap());
        if 我 != 它 { 分岔.push(format!("us={:?}: Rust={:?} Python={:?}", c["us"], 我, 它)); }
    }
    assert!(分岔.is_empty(), "**{} 例分岔**：\n  {}", 分岔.len(), 分岔.join("\n  "));
...
fn uncertainty在上岗档上与python相同() {
    ...
    for g in gold["uncertainty"].as_array().expect("格") {
        let (p, hi, lo, delta) = (g["p"].as_f64().unwrap(), g["hi"].as_f64().unwrap(),
                                  g["lo"].as_f64().unwrap(), g["delta"].as_f64().unwrap());
        let mut store = CalibStore::new();
