...
fn 跑(calib: &CalibStore) -> Result<(Json, Vec<String>, Vec<String>), String> {
    ...
    let mut c = 定值(0.95, RefCell::new(0));
    let mut l = Ledger::new();
    run(&program, &mut c, calib, &a, &mut l)
        .map(|o| (o.value_json(), o.trace.warnings.clone(), o.suspend_candidates.clone()))
        .map_err(|e| e.render())
}
...
fn 漂移信号把线自动标成候选() {
    ...
    观察(&mut c, &近期, None);
    assert!(c.drift_of("k").expect("算得出").可停岗(), "构造的漂移足够强");
    let e = 跑(&c).expect_err("漂移后同一趟里这条线已是候选");
    assert!(e.contains("J-08"), "{e}");
    assert_eq!(c.records["k"].status, "上岗", "内核不改记录，候选经 Outcome 交给宿主落盘");
}
...
"#).expect("解析")).expect("lower");
    let mut cl = 定值(0.95, RefCell::new(0));
    let mut l = Ledger::new();
    let o = run(&program, &mut cl, &c, &ActionRegistry::new(), &mut l).expect("可逆路径照常");
    assert_eq!(o.value_json(), Json::from("act"));
    assert_eq!(o.suspend_candidates, vec!["k".to_string()]);
    assert!(o.trace.warnings.iter().any(|w| w.starts_with("W-suspend-candidate")), "{:?}", o.trace.warnings);
}
