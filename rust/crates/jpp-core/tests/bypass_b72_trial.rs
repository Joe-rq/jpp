//! 绕过测试 B72（步 20f）：试用线出口可路由、报 `W-trial-line`（带 α、n 与「等级不随材料传递」），
//! 不放行不可逆 `do`（J-08 拒）；报告逐出口等级 `Trial`、`releases: false`；只凭账本重放 0 调用、
//! 逐出口等级与首跑逐字节相同（出口不进账本，等级随证书进账本头 `calib_used`）。
...
use jpp_core::{lower, run, run_replay, syntax::parse};
...
fn 跑(src: &str, calib: &CalibStore, l: &mut Ledger) -> Result<jpp_core::Outcome, String> {
    let program = lower(&parse(src).expect("解析")).expect("lower");
    run(&program, &mut 桩(RefCell::new(0)), calib, &动作(), l).map_err(|e| e.render())
}

/// 试用线出 act 供路由，报 W-trial-line（带 α、n、等级不随材料传递）；line_source 说出「试用证书」；
...
fn 试用线可路由() {
    ...
    assert!(
        w[0].contains("α=0.25") && w[0].contains("n=80") && w[0].contains("等级不随材料传递"),
        "{}",
        w[0]
    );
...
fn 只凭账本重放逐出口等级相同() {
    ...
    assert!(
        补回.records.values().any(|r| r
            .certs
            .values()
            .any(|c| c.grade == jpp_core::effects::CertGrade::Trial)),
        "等级随证书进账本头"
    );
    let mut c = 桩(RefCell::new(0));
    let o = run_replay(&program, &mut c, &补回, &ActionRegistry::new(), &mut l)
        .map_err(|e| e.render())
        .unwrap();
    assert_eq!(*c.0.borrow(), 0, "重放不该再调用");
    assert_eq!(
