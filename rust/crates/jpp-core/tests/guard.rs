...
fn 跑(src: &str, p: f64) -> Result<jpp_core::Outcome, jpp_core::Error> {
    ...
    // 可逆：写本地草稿，J-08 不管它
    actions.register("存草稿", 0.0, true, TaintOut::Trusted, |_| Ok(jpp_core::value::Value::text("已存")));
    actions.register("取外部数据", 0.0, true, TaintOut::Untrusted, |_| Ok(jpp_core::value::Value::text("外面来的")));
    let mut client = 定值客户端 { p, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    run(&program, &mut client, &calib, &actions, &mut ledger)
}
...
"#;
    ...
    assert!(t.contains("trusted") || t.contains("可信"), "报文要说清缺什么：{t}");
}
...
#[test]
fn 套一层容器也不能洗白() {
    for (名, 包装) in [("记录字段", "mat({outer: 拆了})"), ("列表", "mat([拆了])"), ("嵌两层", "mat({a: {b: 拆了}})")] {
        let src = format!(
            r#"
budget {{calls: 2, cost: 1, depth: 8}};
let 脏 = do("取外部数据", [], 0);
...
"#
        ...
        assert!(e.render().contains("J-08"), "{名} 该是 J-08：{}", e.render());
    }
}
...
"#;
    // (c) 脏字段当守卫：该拦
    let 用脏 = format!("{头}{{r: if 包.脏字段 {{ content(do(\"发邮件\", [], 0)) }} else {{ \"没发\" }}}}");
    let e = 跑(&用脏, 0.9).expect_err("用不可信判断决定的那个字段当守卫，该被 J-08 拦");
    assert!(e.render().contains("J-08"), "该是 J-08：{}", e.render());

    // (c) 净字段当守卫：该过
    let 用净 = format!("{头}{{r: if 包.净字段 {{ content(do(\"发邮件\", [], 0)) }} else {{ \"没发\" }}}}");
