...
//!（J-02 不变，`derived_from ⊆ sources 的 q_hash 投影`）。预注册：`地基/过程记录/工程-步17c.md` §一第 6 条。
...
use jpp_core::value::{Answer, Mat, Op, Question, State, Taint, Value};
...
fn 跑(src: &str, ledger: &mut Ledger) -> jpp_core::Outcome {
    let program = lower(&parse(src).expect("解析")).expect("lower");
    let mut c = 定值(RefCell::new(0));
    run(&program, &mut c, &库(), &动作(), ledger).expect("跑得完")
}
...
fn f_来源不进哈希与账本键() {
    ...
    let v2 = Value::text("同一段").with_prov(&jpp_core::value::Provenance::new(
        Taint::Trusted,
        jpp_core::value::Sources::from_key("某键"),
    ));
