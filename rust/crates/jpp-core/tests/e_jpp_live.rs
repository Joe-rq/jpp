...
impl Client for 桩 {
    ...
    fn model_id(&self) -> String { "jev-1.13.0".into() }
    fn judge(&mut self, s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
        *self.0.borrow_mut() += 1;
        let answers = qs.iter().map(|q| match q.op {
            ...
            jpp_core::value::Op::Test => Answer::Noul(0.9),
        }).collect();
        let (ms, pm) = qs.iter().map(|q| match q.op {
            jpp_core::value::Op::Select => (Some(1.0), 2),
            _ => (None, 0),
        }).collect::<(Vec<_>, Vec<_>)>();
        Ok(JudgeResult { answers, tokens: 0, cost: 0.0, mode_share: ms, perms: pm })
    }
    fn generate(&mut self, _p: &str, _c: &[Json], n: usize, _r: u64) -> Result<GenResult, EffectError> {
        Ok(GenResult { outputs: (0..n).map(|_| Json::String("门店客流上升，周末为主。".into())).collect(), tokens: 0, cost: 0.0 })
    }
    fn ask(&mut self, _s: &State, _q: &Question) -> Result<Option<Answer>, EffectError> { Ok(None) }
    fn calls(&self) -> u64 { *self.0.borrow() as u64 }
}

fn 报(名: &str, o: &jpp_core::Outcome, l: &Ledger) {
    println!("【{名}】value={} calls={} replayed={} tokens={} usd={:.6} layers={} 证据={}",
             o.value_json(), o.cost.calls, o.cost.replayed, o.cost.tokens, o.cost.usd, o.layers.len(), o.evidence.len());
    for w in &o.trace.warnings { println!("  warn: {w}"); }
    let mut keys: Vec<&str> = l.entries.iter().map(|e| e.key()).collect();
    keys.sort();
    println!("  账本键（{}）: {}", keys.len(), keys.iter().map(|k| &k[..8]).collect::<Vec<_>>().join(" "));
}
...
fn 真机_重跑三次() {
    #[cfg(feature = "live")]
    {
        ...
        for i in 1..=3 {
            ...
            keys.sort();
            println!("第 {i} 次：value={} usd={:.6} tokens={} 请求={} 键={}",
                     o.value_json(), o.cost.usd, o.cost.tokens, c.calls(),
                     keys.iter().map(|k| k[..8].to_string()).collect::<Vec<_>>().join(" "));
            出口.push((o.value_json().to_string(), keys));
            总usd += o.cost.usd; 总tok += o.cost.tokens; 总请求 += c.calls();
        }
        println!("*** 三次合计 usd={:.6} tokens={} 请求={}", 总usd, 总tok, 总请求);
        // **赌 2（最硬的一条）：账本键与模型抖动无关，三次必须逐条对得上**
        assert_eq!(出口[0].1, 出口[1].1, "**账本键三次要一样**——它与模型抖动无关，对不上就是语义差");
        assert_eq!(出口[1].1, 出口[2].1);
        println!("赌 2：账本键三次逐条相同 ✓（与模型抖动无关，这一条成立）");
        let 一致 = 出口.iter().filter(|x| x.0 == 出口[0].0).count();
        println!("赌 1：出口三次里 {一致}/3 相同 → {}", if 一致 == 3 { "无抖动" } else { "有抖动，差异可由抖动解释" });
    }
}
...
fn 真机_单次() {
    ...
    #[cfg(feature = "live")]
    {
        ...
        let o = jpp_core::run(&program, &mut c, &校准(), &动作表(), &mut l).expect("跑得完");
        报("真机·第 1 次", &o, &l);
        println!("  客户端实际发出的请求数 = {}（`cost.calls` 是解释器层的刷新次数；\
        ...
        for (k, sm) in &o.evidence {
            println!("  证据：键={k} p={:?} label={:?} perms={} mode_share={:?} phys={}",
                     sm.p, sm.label, sm.perms, sm.mode_share, sm.phys);
            store.absorb(k, sm.clone()).expect("折得进");
        }
        for k in ["live.noul", "live.choice"] {
            let r = store.get(k);
            println!("  折进之后 {k}：status={} n={} 观察={} 来源={:?} 不可达出口={:?}",
                     r.status, r.n, r.observations(), r.provenance(), r.不可达出口());
        }
