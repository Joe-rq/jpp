...
use jpp_core::effects::{CalibStore, Client, EffectError, JudgeResult};
...
impl Client for 定值客户端 {
    fn model_id(&self) -> String {
        "fixed-0".into()
    }
    fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
        *self.calls.borrow_mut() += 1;
        Ok(JudgeResult { answers: qs.iter().map(|_| Answer::Noul(self.p)).collect(), tokens: 0, cost: 0.0, perms: vec![], mode_share: vec![]  })
    }
    fn generate(&mut self, _p: &str, _c: &[Json], n: usize, _r: u64) -> Result<jpp_core::effects::GenResult, EffectError> {
        *self.calls.borrow_mut() += 1;
        Ok(jpp_core::effects::GenResult { outputs: (0..n).map(|i| serde_json::json!({"第": i})).collect(), tokens: 0, cost: 0.0 })
    }
...
fn 跑(src: &str, p: f64, ledger: &mut Ledger) -> Result<jpp_core::Outcome, jpp_core::Error> {
    let program = lower(&parse(src).expect("解析")).expect("lower");
    let mut calib = CalibStore::new();
    calib.put("k", 0.65, 0.35, 100, "上岗").unwrap();
    let mut client = 定值客户端 { p, calls: RefCell::new(0) };
    run(&program, &mut client, &calib, &ActionRegistry::new(), ledger)
}
...
fn 账本只增不覆盖() {
    ...
    l.put(Entry::Judge { key: "K".into(), answer: Answer::Noul(0.9), tokens: 1, cost: 0.5, model_id: "m1".into() });
    l.put(Entry::Judge { key: "K".into(), answer: Answer::Noul(0.1), tokens: 99, cost: 9.9, model_id: "m2".into() });

    assert_eq!(l.entries.len(), 1, "同键只该有一条");
    let Some(Entry::Judge { answer, tokens, cost, model_id, .. }) = l.get("K") else { panic!("取得到这一条") };
    assert_eq!(*answer, Answer::Noul(0.9), "留下的必须是**先写的那条**，不是后写的");
    assert_eq!((*tokens, *cost, model_id.as_str()), (1, 0.5, "m1"), "费用与模型也不许被改写");
}
...
#[test]
fn 未决责任跨恢复仍找得到() {
    // 预算只够一次调用：第二次 judge 会让程序挂起，此时第一条未决已经产生
    let src = r#"
...
let e2 = cut(r2);
consume(e2, "drop");
{甲: 第一份, 乙: exit_kind(e2)}
"#;
    let mut ledger = Ledger::new();
    // p = 0.5 落在带内 → unsure
    let 第一次 = 跑(src, 0.5, &mut ledger).expect("超预算是挂起不是错");
    assert!(第一次.pending.iter().any(|p| p.cause == "budget"), "该因预算挂起：{:?}", 第一次.pending);
    ...
    assert_eq!(第二次.value_json(), serde_json::json!({"甲": "unsure", "乙": "unsure(band)"}));
...
"#;
    ...
    assert!(t.contains("Bool") || t.contains("真假"), "报文要说清要的是什么：{t}");
}
...
"#;
    ...
    let out = 跑(src, 0.5, &mut Ledger::new())
        .unwrap_or_else(|e| panic!("把未决装进续接方法是合法去向（13 §3），不该被判成责任丢了：{}", e.render()));
...
fn taint解析不出来要往保守那边兜底() {
    use jpp_core::interp::json_to_effect_value;
    use jpp_core::value::Taint;
    use serde_json::json;

    // 正常往返：untrusted 还是 untrusted
    let 正常 = json_to_effect_value(&json!({"__mat": {"x": 1}, "taint": "Untrusted", "addr": "a", "origin": ["lit"]}));
    let jpp_core::value::Value::Mat(m) = 正常 else { panic!("该是材料") };
    assert_eq!(m.taint, Taint::Untrusted, "正常往返不该变");

    // taint 字段坏了（版本变了、被截断、被手改）：**不能兜底成 Trusted**
    for 坏 in [json!("不是合法的 taint"), json!(42), json!(null), json!({"怪": "形状"})] {
        let v = json_to_effect_value(&json!({"__mat": {"x": 1}, "taint": 坏, "addr": "a", "origin": ["lit"]}));
        let jpp_core::value::Value::Mat(m) = v else { panic!("该是材料") };
        assert_eq!(m.taint, Taint::Untrusted, "taint 解析不出来时要当 Untrusted（保守），不是 Trusted（洗白）：坏值 {坏}");
    }

    // taint 字段整个缺失：同样保守
    let 缺 = json_to_effect_value(&json!({"__mat": {"x": 1}, "addr": "a", "origin": ["lit"]}));
    let jpp_core::value::Value::Mat(m) = 缺 else { panic!("该是材料") };
    assert_eq!(m.taint, Taint::Untrusted, "taint 缺失时也要保守");
}
...
"#;
    ...
    actions.register("取外部数据", 0.0, true, jpp_core::TaintOut::Untrusted, |_| Ok(jpp_core::value::Value::text("外面来的")));
    let mut client = 定值客户端 { p: 0.9, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = run(&program, &mut client, &calib, &actions, &mut ledger)
        .unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()));

    let v = out.value_json();
    assert_eq!(v["脏"], serde_json::json!("untrusted"), "do 的输出登记成 Untrusted");
    assert_eq!(
        v["干净"],
...
"#;
    ...
    assert_eq!(v["无辜"], serde_json::json!("trusted"), "源码里的字面量还是可信的：{v:?}");
    assert_eq!(v["也无辜"], serde_json::json!("trusted"), "记录字面量同样：{v:?}");
}
...
"#;
    ...
    assert_eq!(out.value_json(), serde_json::json!({"甲": "act", "乙": "act"}));

    // 两个站点 = 两条账本记录。撞键的话只会有一条，而第二次会记成「重放命中」
    let judges = ledger.entries.iter().filter(|e| matches!(e, Entry::Judge { .. })).count();
    assert_eq!(judges, 2, "两个不同站点问同一道题，该记两条账；撞键会让第二条并进第一条。实际 {judges} 条");
    assert_eq!(out.cost.replayed, 0, "第二个站点不该从账本里命中第一个站点的答案；实际重放了 {} 次", out.cost.replayed);
}
...
fn gen的费用要进预算和账本() {
    ...
    impl Client for 收费生成器 {
        fn model_id(&self) -> String {
            "fixed-0".into()
        }
        fn judge(&mut self, _s: &State, _q: &[&Question]) -> Result<JudgeResult, EffectError> {
            Err(EffectError("这条程序不该 judge".into()))
        }
        fn generate(&mut self, _p: &str, _c: &[Json], n: usize, _r: u64) -> Result<GenResult, EffectError> {
            *self.次数.borrow_mut() += 1;
            Ok(GenResult { outputs: (0..n).map(|i| serde_json::json!({"第": i})).collect(), tokens: 30, cost: self.每次 })
        }
        fn ask(&mut self, _s: &State, _q: &Question) -> Result<Option<Answer>, EffectError> {
            Err(EffectError("不该 ask".into()))
        }
...
"#;
    ...
    let mut client = 收费生成器 { 每次: 0.004, 次数: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger)
        .unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()));

    assert_eq!(out.value_json(), serde_json::json!({"个数": 2}));
    assert!((out.cost.usd - 0.004).abs() < 1e-9, "gen 的实际费用要进 cost.usd，实际 {}", out.cost.usd);
    assert_eq!(out.cost.tokens, 30, "token 也要记");
    let Some(Entry::Effect { cost, .. }) = ledger.entries.iter().find(|e| matches!(e, Entry::Effect { kind, .. } if kind == "gen")) else {
        panic!("账本里该有 gen 这一条")
    };
    assert!((cost - 0.004).abs() < 1e-9, "账本条目也要带实际费用，实际 {cost}");
}
...
fn 改键之后旧账本是miss不是撞错() {
    ...
    let mut l = Ledger::new();
    l.put(Entry::Judge { key: 旧.clone(), answer: Answer::Noul(0.9), tokens: 0, cost: 0.0, model_id: "m".into() });
    ...
    assert_ne!(甲, judge_key("m", "状态哈希", "题哈希", "noul", 1, 0, 100), "perm_seed 要进键");
    assert_ne!(甲, judge_key("m", "状态哈希", "题哈希", "noul", 0, 1, 100), "run_seq 要进键");
}
...
"#;
    ...
    actions.register("取外部数据", 0.0, true, jpp_core::TaintOut::Untrusted, |_| Ok(jpp_core::value::Value::text("外面来的")));
    // Inherit：输出的可信度跟着入参走
    actions.register("原样回传", 0.0, true, jpp_core::TaintOut::Inherit, |args| Ok(args[0].clone()));
    let mut client = 定值客户端 { p: 0.9, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = run(&program, &mut client, &calib, &actions, &mut ledger)
        .unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()));
    ...
    let v = out.value_json();
    assert_eq!(v["脏"], serde_json::json!("untrusted"));
    assert_eq!(v["直接"], serde_json::json!("untrusted"), "直接放参数表里的脏材料本来就认得：{v:?}");
    assert_eq!(v["嵌一层"], serde_json::json!("untrusted"), "记录字段里的脏材料也要认：{v:?}");
    assert_eq!(v["藏在列表里"], serde_json::json!("untrusted"), "嵌套列表里的脏材料也要认：{v:?}");
}
...
"#;
    ...
    let mut client = 定值客户端 { p: 0.9, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect("跑完");
    assert_eq!(out.value_json()["出口taint"], serde_json::json!("trusted"), "{:?}", out.value_json());
}
...
{甲: len(甲), 乙: len(乙)}
"#);
    assert_eq!(n, 2, "gen 的键要带 site（12:158）：两个站点同一段 prompt 该记两条，实际 {n} 条");
...
fn escalate是总次数上限不是每次运行的上限() {
    ...
    impl Client for 有问必答 {
        fn model_id(&self) -> String {
            "fixed-0".into()
        }
        fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
            // p 落在带内 → unsure，程序才会去问人
            Ok(JudgeResult { answers: qs.iter().map(|_| Answer::Noul(0.5)).collect(), tokens: 0, cost: 0.0, perms: vec![], mode_share: vec![]  })
        }
...
"#;
    ...
    let mut calib = CalibStore::new();
    calib.put("k", 0.65, 0.35, 100, "上岗").unwrap();
    let mut client = 有问必答 { 次数: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let _ = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect("第一次跑完");
    assert_eq!(*client.次数.borrow(), 1, "第一次运行问了 1 次人");

    // 把账本里那条 ask 记录去掉 answer 之外的东西不现实；改为直接再跑一次同一本账本：
    // 账本里已有那条 ask，重放该命中、不再问人
    ledger.rebuild_index();
    let mut client2 = 有问必答 { 次数: RefCell::new(0) };
    let _ = run(&program, &mut client2, &calib, &ActionRegistry::new(), &mut ledger).expect("恢复后跑完");
    assert_eq!(*client2.次数.borrow(), 0, "账本里已有的 ask 该重放命中，不重复问人");
    ...
    let mut client3 = 有问必答 { 次数: RefCell::new(0) };
    let out = run(&program2, &mut client3, &calib, &ActionRegistry::new(), &mut ledger).expect("挂起不是错");
    assert_eq!(
        *client3.次数.borrow(),
        0,
        "上限 1 已经在上一轮用掉：恢复后不该再问第二个人。escalate 是总次数上限，不是每次运行 k 次"
    );
    assert!(out.pending.iter().any(|p| p.cause == "budget.escalate"), "该因 escalate 上限挂起：{:?}", out.pending);
}
...
"#
    ...
    let mut ledger = Ledger::new();
    let out = run(&program, &mut client, &calib, &ActionRegistry::new(), &mut ledger).expect("跑完");
    assert!(!out.trace.warnings.iter().any(|w| w.starts_with("W-window")), "不超窗不该报：{:?}", out.trace.warnings);
}
...
"#;
    ...
    assert!(e.render().contains("J-02"), "该是 J-02 禁自指：{}", e.render());
}
...
fn derived_from要能往返账本() {
    use jpp_core::interp::json_to_effect_value;
    use jpp_core::value::{Mat, Taint, Value};
    use std::collections::BTreeSet;

    let mut d = BTreeSet::new();
    d.insert("题哈希甲".to_string());
    d.insert("题哈希乙".to_string());
    let m = Value::Mat(std::rc::Rc::new(Mat::new(serde_json::json!({"x": 1}), "a", vec!["do:k".into()], Taint::Untrusted, d.clone())));

    let j = jpp_core::interp::effect_value_to_json(&m);
    let 回来 = json_to_effect_value(&j);
    let Value::Mat(m2) = 回来 else { panic!("该是材料") };
    assert_eq!(m2.derived_from, d, "derived_from 要往返得回来，否则重放出来的不是同一个程序");
