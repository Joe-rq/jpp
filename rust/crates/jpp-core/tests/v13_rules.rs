...
impl Client for CostlyClient {
    ...
    fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
        *self.calls.borrow_mut() += 1;
        Ok(JudgeResult { answers: qs.iter().map(|_| Answer::Noul(self.p)).collect(), tokens: 7, cost: self.cost_each, perms: vec![], mode_share: vec![]  })
    }
    fn generate(&mut self, _p: &str, _c: &[Json], _n: usize, _r: u64) -> Result<jpp_core::effects::GenResult, EffectError> {
...
fn calib() -> CalibStore {
    ...
    c.put("k", 0.65, 0.35, 100, "上岗").expect("校准记录合法");
    c
}
...
"#;
    let mut client = CostlyClient { p: 0.9, cost_each: 0.0, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = run(&program(src), &mut client, &calib(), &ActionRegistry::new(), &mut ledger)
...
"#;
    ...
    let mut client = CostlyClient { p: 0.9, cost_each: 0.0, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let first = run(&p, &mut client, &calib(), &ActionRegistry::new(), &mut ledger).expect("首跑");
    assert_eq!(client.calls(), 1, "一道题一次调用");

    let mut replay = NoCallClient;
    let again = run(&p, &mut replay, &calib(), &ActionRegistry::new(), &mut ledger)
        .unwrap_or_else(|e| panic!("重放不该发调用：{}", e.render()));
    assert_eq!(again.value_json(), first.value_json(), "重放结果要与首跑一致");
...
"#;
    ...
    let mut client = CostlyClient { p: 0.9, cost_each: 0.01, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = run(&p, &mut client, &calib(), &ActionRegistry::new(), &mut ledger).expect("超预算是挂起不是错");

    assert_eq!(client.calls(), 1, "调用确实发生了");
    assert!(out.pending.iter().any(|x| x.cause == "budget"), "程序该因预算停下：{:?}", out.pending);
    assert!(!ledger.entries.is_empty(), "已完成调用的结果必须被保存，不能连同费用一起丢掉");
    assert!(out.cost.usd >= 0.01 - 1e-9, "实际费用要记进来（记的是事实，不是预估），实际 {}", out.cost.usd);

    // 带该记录恢复：不再请求
    ledger.rebuild_index();
    let mut replay = NoCallClient;
    let again = run(&p, &mut replay, &calib(), &ActionRegistry::new(), &mut ledger)
        .unwrap_or_else(|e| panic!("带记录恢复不该再请求：{}", e.render()));
    assert_eq!(again.cost.calls, 0, "恢复时复用已完成记录，不重复付费");
}
...
fn 第六条_整数越界是指向源码的运行错误() {
    ...
    let 跑 = |src: &str| {
        ...
        let mut client = CostlyClient { p: 0.9, cost_each: 0.0, calls: RefCell::new(0) };
        let mut ledger = Ledger::new();
        run(&p, &mut client, &CalibStore::new(), &ActionRegistry::new(), &mut ledger)
    };
    let 该报错 = |src: &str, 说明: &str| {
        ...
        match 跑(&src) {
            Err(e) => {
                let t = e.render();
                assert!(t.contains("溢出") || t.contains("除以零") || t.contains("越界") || t.contains("取模"), "{说明}：报文要说清是什么边界，实际「{t}」");
                // 位置要落在源码里
                let at = format!("{t}");
    ...
    该报错("(0 - 9223372036854775807 - 1) / (0 - 1)", "最小整数除以负一");
    该报错("1 / 0", "除以零");
    该报错("1 % 0", "模零");
}
...
"#;
    let 跑 = |tail: &str| {
        ...
        let mut client = CostlyClient { p: 0.9, cost_each: 0.0, calls: RefCell::new(0) };
        let mut ledger = Ledger::new();
        let out = run(&p, &mut client, &calib(), &ActionRegistry::new(), &mut ledger)
            .unwrap_or_else(|e| panic!("程序应当跑完：{}", e.render()));
        (out.cost.calls, client.calls())
    };

    // 只造、只返回：一次调用都不该发生
    assert_eq!(跑("{有个方法: 1}").0, 0, "只定义方法，不该有 judge");
    assert_eq!(跑("let f = 造一个(); {有: 1}").1, 0, "返回一个将来会 judge 的方法，返回这一步不产生 judge");
    // 存进记录同样
    assert_eq!(跑("let 盒 = {go: 造一个()}; {有: 1}").1, 0, "把它存进记录也得到相同区分");
    // 真调用了才发生
    assert_eq!(跑("let f = 造一个(); f(\"a\").ok").1, 1, "随后调用该方法才发生 judge");
    assert_eq!(跑("let 盒 = {go: 造一个()}; 盒.go(\"a\").ok").1, 1, "经记录字段调用也发生");
}
...
"#;
    ...
    let 声明了上界 = format!("budget {{calls: 2, cost: 0, depth: 8}};{base}
fn apply(m, f) -> Record !{{judge}} {{ f(m) }}
fn 纯用法(m) -> Record !{{}} {{ apply(m, plain) }}
{{a: 纯用法(mat({{x: 1}})).ok}}");
    let report = check(&program(&声明了上界));
    let d = report.find("E-effect").unwrap_or_else(|| panic!("上界不该被实参缩小：\n{}", report.render()));
    assert!(d.message.starts_with("纯用法"), "{}", d.message);

    // 作者没有声明上界（缺省 = 推断）：这时才按调用点实例化，纯调用点不背 judge
    let 没声明 = 声明了上界.replace("fn apply(m, f) -> Record !{judge} { f(m) }", "fn apply(m, f) { f(m) }");
    let report = check(&program(&没声明));
    assert!(report.is_ok(), "没声明上界时按调用点实例化，纯调用点不该被报：\n{}", report.render());
}
...
"#;
    ...
    let mut client = CostlyClient { p: 0.5, cost_each: 0.0, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    let out = run(&p, &mut client, &calib(), &ActionRegistry::new(), &mut ledger)
        .unwrap_or_else(|e| panic!("包进记录返回、调用者消费，应当跑完：{}", e.render()));
    assert_eq!(out.value_json(), serde_json::json!({"状态": "unsure"}));
    assert!(out.returned_unsure.is_empty(), "已经被调用者消费了，不该还挂着：{:?}", out.returned_unsure);
}
...
"#;
    ...
    let mut client = CostlyClient { p: 0.5, cost_each: 0.0, calls: RefCell::new(0) };
    let mut ledger = Ledger::new();
    match run(&p, &mut client, &calib(), &ActionRegistry::new(), &mut ledger) {
        Err(e) => assert!(e.render().contains("J-05"), "该是 J-05：{}", e.render()),
        Ok(out) => panic!("只取了 状态 字段、把带着未决的 待 字段丢了，这是最后一份承接信息：{:?} / returned_unsure={:?}", out.value_json(), out.returned_unsure),
    }
}
