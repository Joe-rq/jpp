...
use jpp_core::effects::{calib_hash, CalibStore, LabelSource, LiteralMode, Sample};
...
fn 只差标签来源的两条记录哈希必须不同() {
    ...
    let 子集 = 建(LabelSource::选择子集 { 判据: "两个模型都同意".into(), 与对错相关: Some(0.527) });

    // 除了标签来源，线、n、状态、簇单位逐字段相同
    let (a, b) = (全体.get("k"), 子集.get("k"));
    assert_eq!((a.hi, a.lo, a.n, a.status.clone()), (b.hi, b.lo, b.n, b.status.clone()));
    assert_eq!(a.选中的证书().unwrap().cluster_unit, b.选中的证书().unwrap().cluster_unit);

    // **而哈希必须不同**
    assert_ne!(calib_hash(&全体), calib_hash(&子集),
...
fn 缺省是未声明不是全体() {
    let mut c = CalibStore::new();
    装(&mut c, "k");
    assert_eq!(c.get("k").label_source, LabelSource::未声明, "**没人说过就是没人说过**");
    let cert = c.commission("k", 0.45, 0.10, "条").expect("认得动");
    assert_eq!(cert.label_source, LabelSource::未声明, "证书冻结它认证时的那个声明");
}
...
#[test]
fn 相关性未测比声明零更可疑() {
    let 未测 = LabelSource::选择子集 { 判据: "两模型同意".into(), 与对错相关: None };
    let 测了 = LabelSource::选择子集 { 判据: "两模型同意".into(), 与对错相关: Some(0.0) };
...
"#).expect("解析")).expect("lower");
    ...
    let w = 跑(&未声明);
    assert!(w.iter().any(|x| x.starts_with("W-label-source")), "**没人说过标签怎么选的，强出口要留痕**：{w:?}");
    ...
    let w2 = 跑(&声明了);
    assert!(!w2.iter().any(|x| x.starts_with("W-label-source")), "**声明过就不该再响**——这条告警的正确稳态是消失：{w2:?}");
}
...
fn 证书地址不被精度或分隔符撞掉() {
    ...
    // 第五位小数不同 → **地址必须不同**
    assert_ne!(造(0.10000, "条", "甲").addr(), 造(0.10001, "条", "甲").addr(),
