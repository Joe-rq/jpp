...
struct 记序客户端 {
    p: f64,
    日志: Rc<RefCell<Vec<String>>>,
}
impl Client for 记序客户端 {
    fn model_id(&self) -> String {
        "fixed-0".into()
    }
    fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
        for q in qs {
            self.日志.borrow_mut().push(format!("judge:{}", q.text));
        }
...
fn 动作表(日志: &Rc<RefCell<Vec<String>>>) -> ActionRegistry {
    ...
    a.register("取外部数据", 0.0, true, TaintOut::Untrusted, |_| Ok(Value::text("外面来的一段内容")));
    a.register("外部失败", 0.0, true, TaintOut::Untrusted, |_| Err("对方服务超时".into()));
    a
}

fn 跑(src: &str, p: f64, passes: Passes) -> (Result<jpp_core::Outcome, String>, Vec<String>) {
...
fn 推测执行不越过用户函数执行do() {
    ...
    assert!(!日志.iter().any(|x| x.starts_with("do:")), "未走的分支里的 do 被执行了：{日志:?}");
    // 与关掉全部 pass 的结果一致
...
fn 派生文本都不能洗白() {
    for (名, 派生) in [
        ...
        let e = out.err().unwrap_or_else(|| panic!("{名}：派生的不可信内容不该洗白成可信，放行了不可逆 do：{日志:?}"));
        assert!(e.contains("J-08"), "{名} 该是 J-08：{e}");
        assert!(!日志.iter().any(|x| x == "do:发邮件"), "{名}：不可逆 do 被执行了");
    }
}
...
"#;
    ...
    let e = out.err().unwrap_or_else(|| panic!("不可信动作的失败值不该成为可信材料：{日志:?}"));
