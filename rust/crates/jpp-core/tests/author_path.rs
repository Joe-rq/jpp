...
#[test]
fn 固定观察未命中要给出状态与题() {
    let mut c = FixedClient::new();
    let s = State::new(vec![jpp_core::value::Mat::literal(Json::String("材料甲".into()))], vec![], vec![], vec![], false);
    let q = Question::new(Op::Measure, "多大把握", "k", vec!["低".into(), "中".into(), "高".into()]);
    let Err(e) = c.judge(&s, &[&q]) else { panic!("没喂记录就该报错") };

    assert!(e.0.contains("材料甲"), "**要打出状态里到底装了什么**：{}", e.0);
    assert!(e.0.contains("scale") && e.0.contains("高"), "**要打出档位字段叫什么、装了什么**：{}", e.0);
...
fn 修法要标明谁能执行() {
    ...
    impl Client for 桩 {
        ...
        fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
            *self.0.borrow_mut() += 1;
            Ok(JudgeResult { answers: qs.iter().map(|_| Answer::Noul(0.99)).collect(), tokens: 0, cost: 0.0, mode_share: vec![], perms: vec![] })
        }
...
"#).expect("解析")).expect("lower");
        ...
        run(&program, &mut 桩(RefCell::new(0)), calib, &ActionRegistry::new(), &mut l).expect("跑得完").trace.warnings.clone()
    };

    // 冷键 → 作者做得到（写一条上岗记录）
    let w = 跑(&CalibStore::new());
    let cold = w.iter().find(|x| x.contains("calib_line")).expect("有这条");
    assert!(cold.contains("【作者可改】"), "**这一条作者做得到，要说出来**：{cold}");
...
fn j08没触发的两个原因各自独立成立() {
    ...
    impl Client for 桩 {
        ...
        fn generate(&mut self, _p: &str, _c: &[Json], n: usize, _r: u64) -> Result<GenResult, EffectError> {
            Ok(GenResult { outputs: (0..n).map(|_| Json::String("生成的材料".into())).collect(), tokens: 0, cost: 0.0 })
        }
        fn ask(&mut self, _s: &State, _q: &Question) -> Result<Option<Answer>, EffectError> { Err(EffectError("x".into())) }
        fn calls(&self) -> u64 { 0 }
    }
...
"#).expect("解析")).expect("lower");
    ...
    assert!(跑(false).is_ok(), "**gen 无 ctx 时输出是 trusted（∨ 的单位元），守卫可信，J-08 照规矩放行**");
}
...
fn 作者查得到哪些动作不可逆() {
    ...
    impl Client for 桩 {
        fn model_id(&self) -> String { "m".into() }
        fn judge(&mut self, _s: &State, _q: &[&Question]) -> Result<JudgeResult, EffectError> { Err(EffectError("x".into())) }
        fn generate(&mut self, _p: &str, _c: &[Json], _n: usize, _r: u64) -> Result<GenResult, EffectError> { Err(EffectError("x".into())) }
        fn ask(&mut self, _s: &State, _q: &Question) -> Result<Option<Answer>, EffectError> { Err(EffectError("x".into())) }
        fn calls(&self) -> u64 { 0 }
    }
    ...
    assert!(e.contains("发出去（**不可逆**）"), "**不可逆的那些要看得见，那是 J-08 的边界**：{e}");
}
