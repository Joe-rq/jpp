...
//! 用的是第五包交付的 `jpp_core::strength::allocate` 本体，不另写一份 Python——
//! 要验的就是那个实现在真读数上还成不成立。
//!
...
use jpp_core::strength::allocate;
...
fn 档案() -> (f64, f64, (f64, f64, f64)) {
    ...
    let j: Json = serde_json::from_str(&std::fs::read_to_string(p).expect("对照基准在")).expect("合法 JSON");
    let s = j["profile"]["safety_lines"].as_array().unwrap();
    let d = &j["profile"]["delta"];
    (s[0].as_f64().unwrap(), s[1].as_f64().unwrap(), (d["noul"].as_f64().unwrap(), d["choice"].as_f64().unwrap(), d["score"].as_f64().unwrap()))
}
...
fn 错误率(条们: &[条], 复核: &[usize]) -> f64 {
    let n = 条们.len();
    let 错 = 条们.iter().enumerate().filter(|(i, c)| !复核.contains(i) && !c.对).count();
    错 as f64 / n as f64
}
...
fn e_alloc_真读数上不确定度能不能排出错误() {
    ...
    let Ok(text) = std::fs::read_to_string(&path) else {
        println!("跳过 E-ALLOC：读不到 {}（公开发布的目录不带实验数据）", path.display());
        return;
    };
    ...
    let 冷 = {
        ...
        c.profile.safety = (hi, lo);
        c.profile.delta = (dn, dc, ds);
        c
    };
    let 上岗 = {
        let mut c = CalibStore::new();
        c.profile.delta = (dn, dc, ds);
        c.put("noul", 0.66, 0.56, 73, "上岗").unwrap();
        c.set_delta("noul", 0.04).unwrap();
        // choice / score 只有上线，没有下线：按 lo = 0 处理
        c.put("choice", 0.35, 0.0, 74, "上岗").unwrap();
        c.set_delta("choice", 0.04).unwrap();
        c.put("score", 0.59, 0.0, 55, "上岗").unwrap();
    ...
    for line in text.lines().filter(|l| !l.trim().is_empty()) {
        ...
        按题型.entry(format!("{栏}/{题型}")).or_default().push(条 { op, p, 对 });
    }
    ...
    for (轮, calib) in [("冷（那次真跑的实际状态）", &冷), ("上岗（扫出来的线）", &上岗)] {
        println!("\n######## {轮}");
        for (题型, 条们) in &按题型 {
            let n = 条们.len();
            ...
            println!("  {:>4} {:>12} {:>14} {:>10}", "k", "按不确定度", "随机(200种子)", "差");
            for k in ks {
                if k > n {
                    continue;
                }
    ...
    assert_eq!(按题型.len(), 6, "两栏 × 三种题型都该有样本，实际 {:?}", 按题型.keys().collect::<Vec<_>>());
    for (格, 条们) in &按题型 {
