    ...
    for n in [5usize, 7, 20, 73] {
        for k in [0usize, 1, n / 2] {
            ...
            for delta in [0.10f64, 0.05] {
                ...
                if (c - delta).abs() > 2e-3 {
                    坏.push(format!("n={n} k={k} δ={delta}: 返回 p={p:.6}，而 P(X≤k|n,p)={c:.6}（该 ≈ δ）"));
                }
            }
        }
    }
    assert!(坏.is_empty(), "**{} 组不满足定义**：\n  {}", 坏.len(), 坏.join("\n  "));
...
fn certify的覆盖率保证站得住() {
    ...
    for _ in 0..重复 {
        let s: Vec<(f64, bool)> = (0..n).map(|_| { let p = rng.next(); (p, rng.next() < p) }).collect();
    ...
    println!("certify 覆盖率：{重复} 次重复，认证成功 {认证次数} 次，真实错误率超过 ucb 的 {违反次数} 次 = {:.3}（δ={delta}）", 违反率);
    assert!(认证次数 > 重复 / 2, "α=0.45 在这个造数下该常常认得动，否则测的不是覆盖率：{认证次数}/{重复}");
    assert!(违反率 <= 2.0 * delta,
            "**违反率 {违反率:.3} 超过 2δ={:.2}**——要么实现的上界不成立，要么我这个判据设计错了。\
             **先别自裁：扫阈值本身是一次多重比较，而 Clopper–Pearson 是按单次算的。**", 2.0 * delta);
}
...
fn drift的三条性质() {
    ...
    assert!(!d.underpowered, "**200 条对 200 条不是功效不足**（原来的 `underpowered` 在这里报 true）");
    assert!(!d.可停岗());
    // 二、已知移位 → KS 要明显
    ...
    assert!(四.significant && !四.underpowered, "4v4 完全分离确实显著：ks={}", 四.ks);
    // 真正的功效不足：**连完全分离都不显著**，那要 n ≤ 3
    let 三 = drift(&a[..3], &b[..3], 10);
    assert!(三.underpowered, "3v3 时 crit>1，完全分离都不显著——那才叫功效不足");
...
fn cluster_subsample的三条性质() {
    let s: Vec<(f64, bool, String)> = (0..30).map(|i| (i as f64 / 30.0, i % 3 == 0, format!("seg{}", i % 5))).collect();
    // 一、每簇恰一个
    ...
    let 不同 = (0..50u64)
        .map(|sd| cluster_subsample(&s, sd).iter().map(|(p, _)| format!("{p:.6}")).collect::<Vec<_>>().join(","))
        .collect::<std::collections::BTreeSet<_>>();
    assert!(不同.len() > 1, "50 个种子该采出不止一种组合，否则重采样是空转");
    println!("cluster_subsample：5 簇各取 1，同种子可复现，50 个种子采出 {} 种组合 ✓", 不同.len());
}
