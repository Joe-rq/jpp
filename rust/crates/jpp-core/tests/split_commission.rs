...
/// 60 条：同批选线能上岗（每侧 30 条零错误），拆分后每半每侧约 15 条，
/// 不到零错误所需条数（22），按规则停在待核而不是放宽门槛。
#[test]
fn 同批选线能过的样本拆分后停在待核() {
    let mut 同批 = 折入(60);
    assert!(同批.commission_two_sided("k", 0.10, 0.10).is_ok());
    let mut 拆分 = 折入(60);
    match 拆分.commission_two_sided_split("k", 0.10, 0.10, 20260923) {
        Err(Refusal::跑不成(why)) => assert!(why.starts_with("待核") && why.contains("样本不足"), "{why}"),
...
fn 拆分认证写明方式与两半条数() {
    let mut c = 折入(200);
    let cert = c.commission_two_sided_split("k", 0.10, 0.10, 7).expect("认证过");
    ...
    assert!(cert.n_accepted + lower.n_accepted <= sel.n_certify, "认证条数只来自认证半");
