...
/// 让 judge 恒返回某个 p 的桩（给「与 cut 的出口比」那条用）
fn 定值桩(p: f64) -> impl Client {
    struct S(f64, RefCell<u64>);
    impl Client for S {
        fn model_id(&self) -> String { "m".into() }
        fn judge(&mut self, _s: &State, qs: &[&Question]) -> Result<JudgeResult, EffectError> {
            *self.1.borrow_mut() += 1;
...
fn 认证会算出unsure率() {
    ...
    let u = rec.unsure_rate.expect("**认证要算 unsure_rate**：字段文档写的是「标注集上实测」");
    assert!((0.0..=1.0).contains(&u), "是个率：{u}");
    // **手算一遍当独立来源**：证书只界定放行那一侧，`lo = 0`，所以 `Ignore` 不可达，
    // unsure = 「没过 hi + δ 的那些」。δ 用档案的 noul 缺省。
    let (hi, lo) = (rec.hi, rec.lo);
    assert_eq!(lo, 0.0, "前提：证书把 lo 定成 0");
    let d = c.delta_for(&rec, jpp_core::value::Op::Test);
    let 手算 = rec.samples.iter().filter(|s| s.label.is_some()).filter_map(|s| s.p)
        .filter(|p| !(*p >= hi + d) && !(*p <= lo - d)).count() as f64
        / rec.samples.iter().filter(|s| s.label.is_some() && s.p.is_some()).count() as f64;
    assert!((u - 手算).abs() < 1e-4, "算出来的 {u} 与手算 {手算} 对不上（hi={hi} δ={d}）");
}
...
"#).expect("解析")).expect("lower");
    ...
    assert!((ub - c.get("k").unsure_rate.expect("有值")).abs() < 1e-9, "单题的界就是它自己那个率");
}
...
fn 算出来的unsure率写得出也读得回() {
    ...
    let 读回 = CalibStore::load(&dir).expect("**装载器要认得 unsure_rate**（它是结构体字段，认得的字段是算出来的）");
    let r2 = 读回.get("k");
    assert_eq!(r2.status, "上岗", "状态要原样回来");
    assert_eq!(r2.unsure_rate, Some(原值), "**这个数要原样回来**，否则出料等于没出");
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn walk(d: &std::path::Path) -> Vec<std::path::PathBuf> {
    ...
    if let Ok(rd) = std::fs::read_dir(d) {
        for e in rd.flatten() {
            let p = e.path();
            if p.is_dir() { out.extend(walk(&p)); } else if p.extension().is_some_and(|x| x == "rs") { out.push(p); }
        }
    }
    out
}
