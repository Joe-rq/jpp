...
use jpp_core::effects::{CalibStore, Client, EffectError, GenResult, JudgeResult, LiteralMode, Provenance, Sample};
use jpp_core::interp::Interp;
...
impl Client for 定值客户端 {
    ...
    fn calls(&self) -> u64 { *self.次数.borrow() as u64 }
}
...
fn 跑一趟(ledger: &mut Ledger, calib: &CalibStore) -> jpp_core::Outcome {
...
fn 程序能产生证据而不只是消费线() {
    ...
    assert_eq!(out.evidence.len(), 1, "一道题应当产生一条证据：{:?}", out.evidence);
...
fn 宿主手填与程序积累分得开() {
    ...
    c.put("手填", 0.8, 0.2, 200, "上岗").expect("写得进");
    assert_eq!(c.get("手填").provenance(), Provenance::宿主手填);
    assert_eq!(c.get("手填").observations(), 0, "**n=200 而一条证据也没有——这正是以前分不出来的那种记录**");
    ...
    store.put("积累", 0.8, 0.2, 200, "待真值").expect("写得进");
    assert_eq!(store.get("积累").observations(), 3, "**写线不得抹掉已积累的证据**");
    assert_eq!(store.get("积累").provenance(), Provenance::混合, "n=200 而只有 3 条标注");
...
fn 状态只能是那四个() {
    let mut c = CalibStore::new();
    assert!(c.put("k", 0.8, 0.2, 10, "随便写的").is_err(), "四个状态之外的字面串不该进得去");
    for s in ["冷", "上岗", "停岗", "待真值"] {
        assert!(c.put("k", 0.8, 0.2, 10, s).is_ok(), "{s} 是合法状态");
    }
}
