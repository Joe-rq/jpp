...
**档案（`Profile`）**：线与 δ 是**档案字段**，不是代码常数（`12` §1）。
`Profile::load(路径)` 从档案 JSON 读 `lines.safety_default.{hi,lo}` 与
`delta.{noul,choice_prob_chosen,score}.immediate.p99`——**取值路径与 Python 一致，不是抄结果**。
