...
impl LoadedProgram {
    ...
    /// 全程序偏移 → 所在文件与文件内的行、列、字节区间（步 9a 机读诊断用）。
    /// 行列口径与 [`Diagnostic::render`] 相同：行从 1 起，列按字符从 1 起。
    pub fn locate(&self, span: Span) -> Option<Location> {
        let file = self
            .sources
            .iter()
            .find(|f| span.start >= f.offset && span.start <= f.offset + f.text.len())?;
        let src = &file.text;
        let start = (span.start - file.offset).min(src.len());
        let end = span
            .end
        ...
        Some(Location {
            ...
            col: src[line_start..start].chars().count() + 1,
            start,
            end,
        })
    }
