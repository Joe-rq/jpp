...
内核（现行 jpp-core、jpp-syntax）不许出现 home_dir、"~/"、std::env、env::var。
凭据读取属于后端（目标 jpp-backend-jev）；现行 effects.rs 的 live 客户端读 HOME 计为违规，步 15b 移走。
...
hits = []
for f in rust_files("jpp-core", "jpp-syntax"):
