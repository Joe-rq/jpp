...
后接数字。允许名单：jpp-value/src/stat.rs、jpp-calib/src/strength.rs（及其旧位置 jpp-core/src/conformal.rs、strength.rs）与测试。

扫描范围：`crates/` 下除 jpp-cli 以外的全部 crate（2026-09-24 起；此前只扫 jpp-core、jpp-frontend，
拆 crate 时常数随文件移出扫描面，计数下降却没有删常数——步 9 实测 99→95）。新增 crate 自动纳入。
...
ALLOW = ("jpp-value/src/stat.rs", "jpp-core/src/conformal.rs", "jpp-core/src/strength.rs", "jpp-calib/src/strength.rs")
num = re.compile(r"(?<![\w.])(\d+\.\d+|\d+(\.\d+)?e-?\d+)(?![\w])")
named = re.compile(r"(concurrency|window|delta|k_limit|latency)\w*\s*[:=]\s*\d")
hits = []
SCAN = sorted(p.name for p in (ROOT / "crates").iterdir() if p.is_dir() and p.name != "jpp-cli")
for f in rust_files(*SCAN):
