...
| [地基/11-语言规范-v1.md](地基/11-语言规范-v1.md) | Language spec v1.1 (archived as a design study; superseded on the grammar question by the standalone `.jpp` source now implemented in [`rust/`](../rust/README.md)) |
...
| [地基/13-Rust实践反馈设计修订-v0.2.md](地基/13-Rust实践反馈设计修订-v0.2.md) | Six local revisions fed back from the Rust implementation (method construction vs. call, effects in the method contract, where an unresolved exit may go, method identity, budget accounting, integer behaviour). Where it conflicts with `12`, it wins; the rest of `12` stands |
