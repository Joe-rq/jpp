...
- Nature 授权（2026-09-21）：材料来源登记 M1–M9 里的实验材料**可送官方 Jev API**，不再逐份确认；边界仍是「官方 Jev」——送其他外部服务（非 Jev 的 API）仍按原规则先问。haiku 裁判臂走本机 `claude -p`（Nature 自己的 Claude 账号），视为同一边界内，记此判断。E9f-2b′ 预注册里「跑前 Nature 确认文档清单」一条改为已授权。（总控记）〔公开副本脱敏：本条原话及其中点名的材料来源分类未公开——授权范围是可送模型 API，不等于可发公开仓库；原文只留在工作区，工作区未改。〕
