...
十人案例已经可运行：`jpp towow`。它用真实 JEV 响应的离线录制展示“模糊意图 → 转介 → 三方候选 → 新参与者”，并生成可点击的计算轨迹。[查看案例、实测速度与复现方法](docs/towow-demo.zh-CN.md)。
...
公开 alpha 已提供安装入口、运行内核、组合库与离线示例；当前主分支增加了通爻十人案例及真实后端运行记录。研究工作正在完善问题、材料、三种题型和结果的接口，并让更多不同算法接入同一套组件。更广泛的真实后端评估与独立语法是后续里程碑。
